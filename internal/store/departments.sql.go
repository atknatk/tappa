...
SELECT
    (SELECT count(*) FROM employees e
      WHERE e.tenant_id = $1 AND e.department_id = $2::uuid) AS employee_count,
    (SELECT count(*) FROM transactions x
      WHERE x.tenant_id = $1 AND x.department_id = $2::uuid) AS transaction_count
`

type CountDepartmentReferencesParams struct {
	TenantID uuid.UUID
	ID       uuid.UUID
}

type CountDepartmentReferencesRow struct {
	EmployeeCount    int64
...
func (q *Queries) CountDepartmentReferences(ctx context.Context, arg CountDepartmentReferencesParams) (CountDepartmentReferencesRow, error) {
	row := q.db.QueryRow(ctx, countDepartmentReferences, arg.TenantID, arg.ID)
	var i CountDepartmentReferencesRow
	err := row.Scan(&i.EmployeeCount, &i.TransactionCount)
	return i, err
}
...
type DeleteDepartmentRow struct {
	ID         uuid.UUID
	LocationID uuid.UUID
	Name       string
	ShiftStart pgtype.Time
...
func (q *Queries) DeleteDepartment(ctx context.Context, arg DeleteDepartmentParams) (DeleteDepartmentRow, error) {
	row := q.db.QueryRow(ctx, deleteDepartment, arg.TenantID, arg.ID)
	var i DeleteDepartmentRow
	err := row.Scan(
		&i.ID,
		&i.LocationID,
		&i.Name,
...
const getPanelDepartment = `-- name: GetPanelDepartment :one
SELECT d.id, d.tenant_id, d.location_id, d.name,
       d.shift_start, d.shift_end, d.overnight, d.created_at,
       l.name AS location_name
FROM departments d
JOIN locations l ON l.tenant_id = $1 AND l.id = d.location_id
WHERE d.tenant_id = $1
  AND d.id = $2
`

type GetPanelDepartmentParams struct {
	TenantID uuid.UUID
	ID       uuid.UUID
}

type GetPanelDepartmentRow struct {
	ID           uuid.UUID
	TenantID     uuid.UUID
	LocationID   uuid.UUID
	Name         string
...
func (q *Queries) GetPanelDepartment(ctx context.Context, arg GetPanelDepartmentParams) (GetPanelDepartmentRow, error) {
	row := q.db.QueryRow(ctx, getPanelDepartment, arg.TenantID, arg.ID)
	var i GetPanelDepartmentRow
	err := row.Scan(
		&i.ID,
		&i.TenantID,
		&i.LocationID,
...
const listPanelDepartments = `-- name: ListPanelDepartments :many
...
type ListPanelDepartmentsParams struct {
	TenantID uuid.UUID
	RowLimit int32
}

type ListPanelDepartmentsRow struct {
	ID           uuid.UUID
	TenantID     uuid.UUID
	LocationID   uuid.UUID
	Name         string
...
func (q *Queries) ListPanelDepartments(ctx context.Context, arg ListPanelDepartmentsParams) ([]ListPanelDepartmentsRow, error) {
	rows, err := q.db.Query(ctx, listPanelDepartments, arg.TenantID, arg.RowLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListPanelDepartmentsRow{}
	for rows.Next() {
		var i ListPanelDepartmentsRow
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.LocationID,
