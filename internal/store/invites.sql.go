...
const cancelPendingInvitesForEmployee = `-- name: CancelPendingInvitesForEmployee :many
...
type CancelPendingInvitesForEmployeeParams struct {
	TenantID   uuid.UUID
...
func (q *Queries) CancelPendingInvitesForEmployee(ctx context.Context, arg CancelPendingInvitesForEmployeeParams) ([]uuid.UUID, error) {
	rows, err := q.db.Query(ctx, cancelPendingInvitesForEmployee, arg.TenantID, arg.EmployeeID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const consumeInviteAndActivate = `-- name: ConsumeInviteAndActivate :one
WITH consumed AS (
    UPDATE employee_invites
    SET used_at = now()
    WHERE code_hash = $2
...
type ListPendingInvitesForEmployeeParams struct {
	TenantID   uuid.UUID
	EmployeeID uuid.UUID
}

type ListPendingInvitesForEmployeeRow struct {
	ID         uuid.UUID
	TenantID   uuid.UUID
	EmployeeID uuid.UUID
	CreatedAt  time.Time
...
func (q *Queries) ListPendingInvitesForEmployee(ctx context.Context, arg ListPendingInvitesForEmployeeParams) ([]ListPendingInvitesForEmployeeRow, error) {
	rows, err := q.db.Query(ctx, listPendingInvitesForEmployee, arg.TenantID, arg.EmployeeID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListPendingInvitesForEmployeeRow{}
	for rows.Next() {
		var i ListPendingInvitesForEmployeeRow
		if err := rows.Scan(
			&i.ID,
			&i.TenantID,
			&i.EmployeeID,
