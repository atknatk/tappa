...
const tenantHasAnyTransaction = `-- name: TenantHasAnyTransaction :one
SELECT EXISTS (
    SELECT 1 FROM transactions x WHERE x.tenant_id = $1
) AS any_record
`
...
func (q *Queries) TenantHasAnyTransaction(ctx context.Context, tenantID uuid.UUID) (bool, error) {
	row := q.db.QueryRow(ctx, tenantHasAnyTransaction, tenantID)
	var any_record bool
	err := row.Scan(&any_record)
	return any_record, err
}
