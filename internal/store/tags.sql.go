...
const advanceTagCounter = `-- name: AdvanceTagCounter :one

WITH prev AS (
    SELECT p.uid AS old_uid, p.last_ctr AS old_ctr
    FROM tags p
    WHERE p.uid = $3 AND p.tenant_id = $2
    FOR UPDATE
)
UPDATE tags t
SET last_ctr = $1
...
type AdvanceTagCounterParams struct {
	Ctr      int32
	TenantID uuid.UUID
	Uid      string
}

type AdvanceTagCounterRow struct {
	Uid    string
	CtrGap int32
}
...
// sqlc and Postgres.
func (q *Queries) AdvanceTagCounter(ctx context.Context, arg AdvanceTagCounterParams) (AdvanceTagCounterRow, error) {
...
type InsertUnassignedRow struct {
	Uid       string
	CreatedAt time.Time
}
...
// resolve_tag_by_uid.
func (q *Queries) InsertUnassigned(ctx context.Context, arg InsertUnassignedParams) (InsertUnassignedRow, error) {
	row := q.db.QueryRow(ctx, insertUnassigned, arg.Uid, arg.TenantID, arg.AesKeyRef)
	var i InsertUnassignedRow
	err := row.Scan(&i.Uid, &i.CreatedAt)
	return i, err
}
...
const markTagEncoded = `-- name: MarkTagEncoded :one
UPDATE tags
...
type MarkTagEncodedParams struct {
	Uid      string
	TenantID uuid.UUID
}

type MarkTagEncodedRow struct {
	Uid       string
	EncodedAt *time.Time
}
...
func (q *Queries) MarkTagEncoded(ctx context.Context, arg MarkTagEncodedParams) (MarkTagEncodedRow, error) {
	row := q.db.QueryRow(ctx, markTagEncoded, arg.Uid, arg.TenantID)
	var i MarkTagEncodedRow
	err := row.Scan(&i.Uid, &i.EncodedAt)
	return i, err
}
...
const unmountTagFromWall = `-- name: UnmountTagFromWall :one
UPDATE tags
...
type UnmountTagFromWallRow struct {
	Uid        string
	TenantID   uuid.UUID
	LocationID *uuid.UUID
	LastCtr    int32
...
func (q *Queries) UnmountTagFromWall(ctx context.Context, arg UnmountTagFromWallParams) (UnmountTagFromWallRow, error) {
	row := q.db.QueryRow(ctx, unmountTagFromWall, arg.TenantID, arg.Uid)
	var i UnmountTagFromWallRow
	err := row.Scan(
		&i.Uid,
		&i.TenantID,
		&i.LocationID,
