...
const closeBillingPeriod = `-- name: CloseBillingPeriod :one
WITH scope AS (
...
type CloseBillingPeriodParams struct {
	ClosedBy    uuid.UUID
	PeriodMonth pgtype.Date
	TenantID    uuid.UUID
}

type CloseBillingPeriodRow struct {
	ID                 uuid.UUID
	TenantID           uuid.UUID
	PeriodMonth        pgtype.Date
...
func (q *Queries) CloseBillingPeriod(ctx context.Context, arg CloseBillingPeriodParams) (CloseBillingPeriodRow, error) {
	row := q.db.QueryRow(ctx, closeBillingPeriod, arg.ClosedBy, arg.PeriodMonth, arg.TenantID)
	var i CloseBillingPeriodRow
	err := row.Scan(
		&i.ID,
		&i.TenantID,
		&i.PeriodMonth,
...
const firstChargeableMonthShift = `-- name: FirstChargeableMonthShift :one
SELECT tappa_first_chargeable_month(t.plan, t.created_at, t.timezone)::date       AS current_month,
       tappa_first_chargeable_month(t.plan, t.created_at, $1::text)::date  AS proposed_month
FROM tenants t
WHERE t.id = $2
`

type FirstChargeableMonthShiftParams struct {
	Timezone string
	TenantID uuid.UUID
}

type FirstChargeableMonthShiftRow struct {
	CurrentMonth  pgtype.Date
	ProposedMonth pgtype.Date
}
...
func (q *Queries) FirstChargeableMonthShift(ctx context.Context, arg FirstChargeableMonthShiftParams) (FirstChargeableMonthShiftRow, error) {
	row := q.db.QueryRow(ctx, firstChargeableMonthShift, arg.Timezone, arg.TenantID)
	var i FirstChargeableMonthShiftRow
	err := row.Scan(&i.CurrentMonth, &i.ProposedMonth)
	return i, err
}
...
type GetBillingPeriodParams struct {
	TenantID    uuid.UUID
	PeriodMonth pgtype.Date
}

type GetBillingPeriodRow struct {
	ID                 uuid.UUID
	TenantID           uuid.UUID
	TenantName         string
	PeriodMonth        pgtype.Date
...
func (q *Queries) GetBillingPeriod(ctx context.Context, arg GetBillingPeriodParams) (GetBillingPeriodRow, error) {
	row := q.db.QueryRow(ctx, getBillingPeriod, arg.TenantID, arg.PeriodMonth)
	var i GetBillingPeriodRow
	err := row.Scan(
		&i.ID,
		&i.TenantID,
		&i.TenantName,
...
func (q *Queries) ListBillingPeriods(ctx context.Context, arg ListBillingPeriodsParams) ([]ListBillingPeriodsRow, error) {
	rows, err := q.db.Query(ctx, listBillingPeriods, arg.TenantID, arg.RowLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const previewBillingPeriod = `-- name: PreviewBillingPeriod :one
...
type PreviewBillingPeriodParams struct {
	PeriodMonth pgtype.Date
	TenantID    uuid.UUID
}

type PreviewBillingPeriodRow struct {
	TenantID             uuid.UUID
	TenantName           string
	Plan                 string
	Timezone             string
...
func (q *Queries) PreviewBillingPeriod(ctx context.Context, arg PreviewBillingPeriodParams) (PreviewBillingPeriodRow, error) {
	row := q.db.QueryRow(ctx, previewBillingPeriod, arg.PeriodMonth, arg.TenantID)
	var i PreviewBillingPeriodRow
	err := row.Scan(
		&i.TenantID,
		&i.TenantName,
		&i.Plan,
