...
// *config.Config and a list of Mounters and NOTHING else, so a handler registered
// by this package has no pool, no session codec and no audit sink to reach.
//
...
func TestHealthz_CannotDependOnAnything(t *testing.T) {
	...
	if fn.NumIn() != 2 || !fn.IsVariadic() {
		t.Fatalf("NewRouter takes %d parameters (variadic=%v); liveness rests on it taking a config and mounters only",
	...
	if got := fn.In(1).String(); got != "[]httpx.Mounter" {
		t.Errorf("NewRouter's second parameter is %s, want []httpx.Mounter", got)
	}
	...
	srv := httptest.NewServer(httpx.NewRouter(nil))
	t.Cleanup(srv.Close)
