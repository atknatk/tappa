...
// nothing reactivates anybody — the product's only irreversible action was one
	...
	attacks := []struct {
		...
		{"a genuine token re-signed under a guessed key", func(t *testing.T, b *browser) (string, string) {
			...
			forged, err := adminConfirm{key: []byte("not-the-servers-key"), ttl: adminConfirmTTL}.
				mint(employeeID, panelTestSession)
			if err != nil {
...
func TestAdminConfirm_IsBoundAndExpires(t *testing.T) {
	...
	token, err := c.mint(employee, session)
	if err != nil {
		t.Fatalf("mint: %v", err)
	}
	// POSITIVE CONTROL FIRST.
	if err := c.parse(token, employee, session); err != nil {
	...
	if err := c.parse(token, employee, uuid.New()); !errors.Is(err, errConfirmInvalid) {
	...
	if err := c.parse(token, uuid.New(), session); !errors.Is(err, errConfirmOtherPerson) {
	...
	if err := aged.parse(token, employee, session); !errors.Is(err, errConfirmInvalid) {
	...
	if err := fresh.parse(token, employee, session); err != nil {
	...
	if err := backwards.parse(token, employee, session); !errors.Is(err, errConfirmInvalid) {
	...
	if err := jitter.parse(token, employee, session); err != nil {
	...
	foreign, err := otherKey.mint(employee, session)
	if err != nil {
		t.Fatalf("mint under the other key: %v", err)
	}
	if err := c.parse(foreign, employee, session); !errors.Is(err, errConfirmInvalid) {
