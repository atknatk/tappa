...
func TestReviewDecision_AMalformedFormNeverLogsWhatWasSubmitted(t *testing.T) {
	...
	records := queueWith(1)
	reviewer := &fakeReviewer{}
	h, err := NewAdminAuth(admins, &fakeTrail{}, records, records, reviewer, &fakeStaff{}, &fakeInviter{}, &fakeVenues{}, &fakePlaques{}, &fakeRecorder{}, newFakeRules(), adminTestConfig(), logger)
	if err != nil {
...
func TestSameOriginRefusal_NamesTheRouteItRefused(t *testing.T) {
	...
	h, err := NewAdminAuth(admins, &fakeTrail{}, records, records, &fakeReviewer{}, &fakeStaff{}, &fakeInviter{},
		&fakeVenues{}, &fakePlaques{}, &fakeRecorder{}, newFakeRules(), adminTestConfig(), logger)
	if err != nil {
		t.Fatalf("NewAdminAuth: %v", err)
	}
	r := chi.NewRouter()
