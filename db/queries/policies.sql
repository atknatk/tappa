...
LEFT JOIN policy_versions v
       ON v.tenant_id = @tenant_id AND v.policy_id = p.id
LEFT JOIN admin_users a
       ON a.tenant_id = @tenant_id AND a.id = v.created_by
WHERE p.tenant_id = @tenant_id
...
VALUES (@tenant_id, @name, 'tenant', @enabled, @created_by)
RETURNING id, name, layer, enabled;

-- name: NextPolicyVersionNo :one
...
VALUES (@tenant_id, @policy_id, @version_no, @document, @created_by)
RETURNING id, version_no, created_at;

-- name: AttachPolicyResource :one
...
WHERE tenant_id = @tenant_id AND policy_id = @policy_id AND resource = @resource
RETURNING id, resource;

-- name: GetPolicyVersionDocument :one
...
JOIN policies p
  ON p.tenant_id = @tenant_id AND p.id = v.policy_id
LEFT JOIN admin_users a
  ON a.tenant_id = @tenant_id AND a.id = v.created_by
WHERE v.tenant_id = @tenant_id AND v.policy_id = @policy_id AND v.version_no = @version_no;

-- name: ListPolicyVenueTargets :many
...
-- name: ListPolicyDepartmentTargets :many
...
SELECT EXISTS (
    ...
    WHERE tenant_id = @tenant_id AND layer = 'tenant' AND name = @name
) AS taken;

-- name: RenameTenantPolicy :one
...
UPDATE policies
SET name = @name
WHERE tenant_id = @tenant_id AND id = @id AND layer = 'tenant'
RETURNING id, name;
