...
| **Sıradaki görev** | **M7-04 FAZ B — handler, ekran, e-posta taşıyıcısı, oran sınırı. FAZ A DONE (`b039ef3`).** 🔴 **B'NİN KABUL KRİTERLERİ ARTIK KARTTA YAZILI** — A fazının ölçümlerinden doğdular ve **denetlenecek liste orasıdır** (`m7-portal.md` → M7-04 → *"Kabul kriterleri — B FAZI"*): **(1) sıfırlama isteği ADRES BAŞINA oran sınırlı** — tek sayı **iki** zararı kapatıyor: **kurtarma reddi** (saldırgan kurbanın adresini yazıp bekleyen linkini iptal ettirir, tekrarlanırsa kurtarma **hiç tamamlanamaz**) ve **CPU tükenmesi** (`Consume` token'ın varlığına bakmadan **212,8 ms** bcrypt öder). ⚠️ **Sıra değiştirilerek çözülemez** — önce çözümlemek bir **zamanlama kehaneti** açar. **(2)** başarısız her deneme **bir yere düşüyor** (§4.6): çözümlenen → `audit_log`; çözümlenmeyen → süreç log'u (çünkü `audit_log.tenant_id` NOT NULL ve o vakada tenant yok); **hiçbirinde token/hash yazılmaz**. **(3)** link **yalnız yöneticinin KENDİ satırındaki adrese** gider — veri katmanı **minting'i kapatmaz ve kapatamaz**, ele geçirmeyle arasında duran tek şey bu. **(4)** bir `store.*Params` **asla yazdırılmıyor** (ham token redakte eder, **hash etmez** — depo geneli desen). **(5)** e-postadan adaya eşleme bir **numaralandırma kehaneti değil**; aday daraltma kararı buradadır ve **yanlış verilirse 00017'nin artık-kilidini bu yüzeyde yeniden üretir**. 🔴 **Q02 KULLANICIDA VE B'NİN GERÇEK ÖNKOŞULU** — orkestratör önerisi **Postmark + AB ikameti** (hacim önemsiz, teslim edilebilirlik hayati: spam'e düşen davet = **aktive olamayan çalışan**); kendi SMTP elendi (yeni IP itibarsız), SES elendi (**AWS'yi ikinci altyapı bağımlılığı** yapıyor, §1'e ters). ⚠️ **Taşıyıcı tek metotlu bir TÜKETİCİ-TARAFI arayüzün arkasında olmalı** (§7) — o zaman sağlayıcı değiştirmek bir dosya olur ve Q02 geri alınabilir bir karara döner. **VE Q02 BİR ADR 0005 RİSKİNİ DE KAPATIYOR:** `open-questions.md:52` **Y-D** (*"müdür kimlik basabiliyor: davet kodu panelde görünüyor"*) kapanışını *"Q02 çözülünce kod çalışanın kendi kanalına"* diye yazıyor. **⚠️ ÖLÇÜM ZEMİNİ (2026-08-14):** `make test` **3307 PASS / 0 FAIL / 0 SKIP · 19 paket**, duvar **234–565 s** (**tamamen yüke bağlı**; `internal/handler` ~256 s **bilinçli ve kabul edilmiş**, **yeniden açma**); **çıplak `go test` DB testlerini SESSİZCE atlar** — **SKIP sayısını HER ZAMAN yazdır**; **`.templ` sonrası `make fmt gen`**; **her sayıya `uptime` yaz**. `make audit` **exit 2**, sebebi govulncheck (**T31, kullanıcının işi**). Son migration **00019**, son ADR **0015**. **Canlı iddia olarak sayı/tarih yazma.** |
