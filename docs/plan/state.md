...
**Son güncelleme:** 2026-09-26 (27. oturum, dördüncü yarı — ✅ **M10 A1 OP-5 DONE** (`6841d68`, dal `m10-a1`): migration **00026** (`platform_admins`, `platform_sessions`, `operator_audit_log`, `operator_read_tickets`) + roller `01-roles.sql`'de + beş `op_*` definer; 5 yapıcı turu, üçüncü göz RED → ONAY, güvenlik ONAY, kapanış ONAY · 🔴 **deploy ön koşulu:** canlıda rol runbook'u main'e birleştirmeden ÖNCE · **sıradaki: OP-6** · **önceki:** 2026-09-26 (27. oturum — ✅ **M10 FAZ 0'IN KOD KISMI BİTTİ**: F0-6 mount kapısı + OP-3 (migration 00024, `tenants` UPDATE daraldı + DELETE kalktı) + F0-6b (migration 00025, `active`'e geçiş trigger'ı); F0-5 kesinti testi CI'da gerçek bir TMPDIR sızıntısı buldu ve kapandı; **main'e birleştirme = deploy = kullanıcı kararı** · **ilk parti:** dal `m10-faz0`'da, pushlu: **F0-5 sır kapısı** (redline R7d + pre-push kancası, muafiyet satırın sha256'sına bağlı; 12 yapıcı / 12 denetim, 7 RED) · F0-7 (writedata+91AE → already-encoded) · OP-2 · 🔴 **main CI `b09ba1e`'den beri kırmızıydı** (plan belgesinde var olmayan test atfı; deploy atlandı, canlı etkilenmedi) — düzeltme `e226eea` dalda · F0-6 sürüyor) · **önceki:** 2026-09-24 (26. oturum — 🔴 **OLAY A-0: operatör panel parolası state.md'de düz metin + depo PUBLIC** (değer HEAD'den kaldırıldı `920baaa`; triage: yetkisiz kullanım kanıtı YOK; ROTATE kullanıcıda) · 📋 **M10 PLANI** (`docs/plan/m10-platform.md`): Faz 0 acil güvenlik → A1 operatör kimliği (ayrı `platform_admins` + TOTP + `op_*` definer) → B e-posta (SES, `net/smtp`) → C white-label (tek accent + logo) → A2 tenant-ötesi araçlar · kullanıcının endişesi denetlendi: **kodda yetki açığı YOK**, sorun yapısal (platform gücü bir restoran hesabında) · D-A..D-D karara bağlandı (public kalır · ayrı operatör · tap ekranında marka · operatör→SES→WL) · kullanıcı pilotu kurmuş (2 yeni çip encode, 8 lokasyon, 3 çalışan) — *(önceki: 2026-09-20 (25. oturum — ✅ **md.5 DONE: ANAHTAR-0 (AppMasterKey) ROTASYONU / STEP 8 SEVK EDİLDİ** (`95cf32d`, dal `md5-key0-rotation`, T73'ün ucunda) · ✅ **T73+md.5 CANLIYA DEPLOY EDİLDİ** (healthz/readyz 200) · ✅ **ÇİFT-TENANT TEMİZLENDİ → tek temiz "Kebab Factory Ltd."** (VAT MT22339606 VIES-onaylı, admin `f3c91606`, tenant `4931dd67`; iki eski dublicate signup silindi) · ✅ **MinPasswordRunes 12→8** (ADR 0019) + operatör allow-list yeni id — **canlı main `aefa67d`** · ✅ **ENCODE HATA-LOGLAMA + UI SEBEP + Android 11-exchange** (`966df96`+`aefa67d`; kullanıcı ÖNCEKİ çibi tekrar-encode'da 911E aldı — yapılandırılmış log + `already-encoded` fault + md.5 Android borcu kapandı; iki denetim ONAY) · 🔴 canlı tek hesap `ozcan@kebabfactory.com.mt` / `[parola repodan kaldırıldı]` (T73 ile set) · ✅ flaky **gpsredaction testi** (T65 + geo twin) düzeltildi (`29842dd`) · ⚠️ **DERS:** gofmt/gen atlaması CI'yı bir kez kırdı — orkestratör commit öncesi en az `gofmt -l`+`make gen` koşmalı) · **ADR 0018** (anahtar-0 ayrı sarmalı sütun `tags.app_key_ref`, migration 00023, yaz-bir-kez; elenen KDF/yaz-at şıklarının fiyatıyla) · encode driver'a **step 8** (`ChangeKey(0)`, en son, oturumu bitirir) · iki bağımsız anahtar (key-0 ≠ key-1), §5.2 DB-önce, fake chip oturum-ölümü modeli · **iki denetim ONAY** (üçüncü göz + tappa-security-auditor §4.7; F1 app-zarfı negatif testi + F2 dangling notu kapatıldı, F1 mutasyonla kanıtlandı) · 🔴 **KOD + FAKE-CHIP; GERÇEK ÇİPTE step 8 AYRI/GERİ-DÖNÜLEMEZ ADIM** (çip fabrika anahtarını kaybeder, kullanıcı kararı) · **SIRADAKİ İŞ 3: Q02** (e-posta, sağlayıcı kararı bekliyor) · ⚠️ yeni pre-existing sapma: `cmd/rotatekek` GOPACKAGESDRIVER (Go 1.27 toolchain, T72 sınıfı) — *(önceki: 2026-09-20 (24. oturum — ✅ **T73 DONE: YÖNETİCİ PANELDEN KENDİ PAROLASINI DEĞİŞTİREBİLİYOR** (`38ddbaf`, dal `t73-admin-change-password`, **main'e MERGE EDİLMEDİ** = deploy kullanıcı kararı) · herhangi bir canlı admin (owner+manager) mevcut parolasını doğrulayıp değiştirir · **veri katmanı migration'sız** (3 sqlc sorgu, GRANT'lar 00006/00011/00017'de mevcut, `admins.sql` header invariant grep-dürüst güncellendi: hash artık TAM İKİ yerde SELECT edilir) · **K-A** kapı `id.Live()` (owner-only DEĞİL, parola kişisel) · **K-B** mevcut parola bcrypt Compare · **K-C** yeni≠mevcut · **K3** mevcut oturum HARİÇ diğerleri iptal · **iki denetim ONAY** (üçüncü göz + tappa-security-auditor, tenant izolasyonu belt+RLS, sıfır sır sızıntısı) · **SIRADAKİ İŞ 3: Q02** (e-posta, sağlayıcı kararı bekliyor) · 🔴 canlı DB şifreleri + Ozcan panel parolası (`[parola repodan kaldırıldı]`) sohbette açık → ROTATE — *(önceki: 2026-09-19 (23. oturum — 🎉 **İLK ÇİP ENCODE EDİLDİ VE UÇTAN UCA TAP DOĞRULANDI** · gerçek NTAG 424 DNA, Galaxy A15 + Android rölesiyle encode edildi (`encoded_at` damgalı), sonra personel "Ozcan Test" olarak tap → **`verdict=ok`, `sun_valid=true`, ctr 6→10** · **iki silikon düzeltmesi:** `fd1b667` (plain WriteData — fabrika NDEF dosyası CommMode.Plain ister) ve `4dd6959` (CmdCtr — gerçek çip plain WriteData'yı SAYIYOR), ikisi de üçüncü göz + güvenlik ONAY · 🔴 tap motoru **HER §5 kararını gerçek tap'te verdi** (practice · debounce · gps-conflict · ip-or-gps-ok) · ⚠️ tuzak: NFC NDEF URL'yi VARSAYILAN tarayıcı açar · **md. 16 KAPANDI** · **SIRADAKİ: md. 5** (anahtar 0 şeması, kullanıcı kararı) · 🔴 canlı DB şifreleri (`tappa_app`+`tappa_owner`) sohbette açığa çıktı → ROTATE · üretim tenant'ında test verisi kalıcı — *(önceki: 2026-09-18 (22. oturum — ✅ **M8-05 FAZ B3a DONE: ANDROID APDU RÖLESİ** (`4fc5e3f`) · Kotlin, `android/`, Go modülünün dışında, **tek runtime bağımlılık `kotlin-stdlib`** · giriş WebView + `CookieManager`, tur `IsoDep` reader thread'inde, on alışveriş · **33 JVM testi / 17 mutasyon / 0 hayatta kalan** · 🔴 **Galaxy A15'e KURULDU ve AÇILDI**, `Origin` telefonda kanıtlandı · 🔴 **üçüncü göz: `ContractPinTest` sıcak ağaçta KÖRDÜ** (Go dosyaları Gradle girdisi değildi, altı sapma yeşil geçti) → kapatıldı · 🔴 **güvenlik: *"holds no secret"* ADR 0017 §3'le çelişiyordu** — telefon **anahtar tutmaz ama sırsız değil**, debug APK'da `am dumpheap` root'suz (ölçüldü) → **sayıldı** · **SIRADAKİ: B3b — GERÇEK ÇİP, DONANIM KULLANICIDA**; ilk tur md. 16'yı kapatır, **üretime yazar**, çip laboratuvar plaketi olur (md. 5 açık) · ⚠️ yerel Go 1.27.1 sapması → **T72**) — *(21. oturum, 2026-09-15: 🟢 **BİREBİR KOPYA + SIGN-IN + SHOW-PASSWORD COMMIT'LENDİ VE `main`'E ALINDI — CANLI** (dal `landing-redesign`: `cc9f0c5` sign-in, `898ef72` landing, bu not; `main`'e birleştirme = deploy, hash'ler ledger altında). **Kullanıcı kararları (bu oturum):** *"sign-in'e show password"* · *"önerdiğin şekilde ilerle, birebir olmasa da olur, küçük değişiklikler kabul"* (R1 ve iddia düzeltmeleri için) · *"devam et, bitince maine push et canlıya alalım"*. 🔴 **KULLANICI KARARI DEĞİŞTİ, KAYDI BURADA:** 20. oturumun *"pilot şeridi pilot başlayana kadar YOK"* kararı, kullanıcının **kendi tasarımını birebir** isteme kararıyla (2026-09-13, *"sadece aynısını yap"*) **geçersiz** — şerit (*Piloting with teams in Malta — Kebab Factory / Kebab Manufacturing*) kullanıcının **kendi işletmesi hakkındaki beyanı**, kodla bağlanamaz, aynen duruyor; ledger'daki cihaz-sütunu hükümleri (*"goes blind"*, *"refuse kitchen hands"*) da tasarımın kendisi, dipnot (*not a benchmark*) altında. Denetçi bunları RED saydı; orkestratör kullanıcı beyanı diye **bilerek** bıraktı. · **Show-password:** düğme markup'ta `hidden`, `adminpassword.js` açıyor (script yok → düğme yok, ölçüldü CDP `setScriptExecutionDisabled`); `adminLoginCSP = adminCSP + script-src 'self'` **yalnız** `/admin/login` 200+401 (tel üzerinden 6 rota ölçüldü). Yapıcı iki gerçek kusuru ekran görüntüsüyle buldu (`svg.hidden` property hiçbir şey yapmıyor; `field.focus()` klavyeyi kırıyordu). · **Denetim harness'i 3 kez üst üste ilk mesajda takıldı** ("no progress for 600s") — üçü de ağaca dokunmadı (`git status` ile ölçüldü); orkestratör denetimi **kendi yaptı**: iki bayat rozet yorumu (benim eksiğim, rozet kaldırılınca güncellenmemiş) · **`make audit` R2 FAIL** (`adminview.go` yorumunda `geofence` kelimesi — satır-yerel tarayıcı; CI'yı kıracaktı) · `adminlogin_test.go`'da `connect-src` kuralı *"her script"* → *"vendored script ⇔ connect-src"* — **ölçülmüş takas**: kendi yazdığımız istek atan bir panel script'i artık yakalanmaz (bugün yok), karşı yön kazanıldı. Sonraki ajanlar (kısa brief'lerle) çalıştı. · 🔴 **BİREBİR KOPYA `make audit`'i R1'DE KIRIYORDU** (HEAD'de değil, çalışma ağacında — CI **yeşil** görünüyordu çünkü kopya commit'lenmemişti). Karar (a) **muafiyeti kelimeye değil CÜMLEYE bağlamak**: `R1_WAIVER_PHRASE` → `R1_WAIVER_PHRASES` (5 literal, `index`/`substr`, regex değil); metin bir harf değişirse muafiyet düşer (P6). İlk yazım **sessizce boş döndü** — awk programı tek tırnak içinde, yorumdaki kesme işareti programı kesti; yakalayan WARN bloğunun **kaybolması** oldu. Aynı tuzağa **iki kez** basıldı. Güvenlik denetçisi GREEN + 2 düşük: boş giriş BSD awk'ta **sonsuz döngü**/gawk'ta sessiz → `exit 2`; ve `$(...)` içinde kaybolan awk kodu → `SCAN_ERR` işaretçisi (ölçüldü: mesaj basılıp exit 0 dönüyordu). Metin veriye taşınınca `>biometric data<` eşleşmesi düştü ve yapıcı kelimeyi değiştirmek zorunda kaldı → muafiyet `"biometric data"` (Go literal) olarak yeniden bağlandı, kullanıcının kelimesi geri geldi. ADR 0012 ek bölüm. · **17 kırık landing testi → 0** (`marketingdemo_test.go` yeni; nav testi `#proof` yokken **panic** atıyordu, düzeltildi): kopya `landingview.go`'da **veri** oldu, her satır pinli; **22 metin düzeltmesi** (API/CSV import/live headcount/unlimited/ölçülmemiş saniyeler/*"IP or GPS"* → §5 birebir/*"have to agree"*/*"Book a pilot"* düğmesi **kaldırıldı** — mailto sahipsiz, servis yok/*"no buddy punching"* → *"the phone has to be there"*/*"verified IP"* → *"network address on file"*/SSS etiketi docket'in gerçek kelimesi *"Entered by a manager"*); ledger dipnotu geri; footer 4 yasal link; `#8FD3AF` → `--mint` token. €90 = 60 × 1,50 **koddan** (`exampleTeamMonthly`, cent tamsayısı, şemaya bağlı). **33 mutasyon kırmızı** (yapıcı) + **6/6** (denetçi bağımsız). Servis edilen HTML diff'i: yalnız metin düzeltmeleri + dipnot + yasal nav + SVG hex→class. · **Ölçümler (go1.26.6):** `make test` **25/25 paket, exit 0** (`-race`) · `make lint` temiz · `make audit` **exit 0** (redline **koştu**, 8 R1 WARN) · `make gen` idempotent · `/` CDP: 0 CSP ihlali, CTA tek düğme · `make check` çalışma ağacında `git diff --exit-code` yüzünden geçemez, parçaları ayrı ayrı yeşil. · ⚠️ **Süreç:** `make audit` yerelde `rg` ister; kabuk `rg`'si Claude'un shim fonksiyonu, `make` onu görmez → scratchpad'de sarmalayıcı (`rgbin/rg`). Sonuç: `make check` **audit'i içermez**, ikisini de koş. · **Kalan/backlog:** T72 (panel *"window is yours to set"*) · T73 (marka kapısı) · R1 tetikleyici listesine toplama API'leri (`capture=`, `navigator.credentials`) — denetçi önerisi, ayrı karar · `adminlogin_test.go` connect-src kuralını *"istek atan script"* üzerinden sıkılaştırma · mint aksan/`green-lite` paleti · IBM Plex Mono 500/600 self-host · SSS *"Piloting"* şeridi pilot gerçekten başlamadıysa kullanıcı kaldırır. · **Ledger DEĞİŞMEDİ** — "ŞU AN" hâlâ M8-05 FAZ B3.)*)*

> *(20. oturum, 2026-09-14 — 🟢 **LANDING: KULLANICININ KENDİ TASARIMI İSKELETE TAŞINDI — DONE, 5. TUR DENETİM ONAY** (`25c6815`, dal `landing-redesign`; `main` `40bc88c`'de, üretim etkilenmedi). Kullanıcı kendi landing HTML'ini verdi (`docs/design/landing-reference-2026-09-12.html`, `3214a5f`) ve *"bunu uygulayalım"* dedi. **İki kullanıcı kararı:** tasarımı mevcut 4-tur-denetimli iskelete **taşı** (görünüm/yapı/enerji/ses dosyadan; iddia disiplini + palet + self-host font + gerçek rotalar mevcut sayfadan; kanıt/yargı bölümleri korunur) · *"Piloting with Kebab Factory (9) / Kebab Manufacturing (5)"* şeridi **pilot başlayana kadar YOK** (seed tenant'ları; M8-06 başlamadı). **Orkestratörün üç varsayımı (söylendi, itiraz gelmedi):** marka **Taptime** · **plaket vaatleri yok** (encode edilmedi, M8-05 B3) · **SSS'deki QR cümlesi §5'e çekildi** (dosya *"IP or GPS"* diyordu; QR'da IP zorunlu, `base:qr-requires-ip`). · **Taşınanlar:** yapışkan nav + `#how/#proof/#pricing` + Start · iki sütunlu hero, sağda **çizilmiş sahne** (koyu `ink` plaket + önünde 2° eğik basılan adisyon, `docket-print` **tek seferlik**) · satış sesi (*"in the lunch rush"*, *"wet or floury hand"*, *"Nothing per device — there are none"*) · koyu `ink` **ledger** (mekanizma karşılaştırması + dipnot, ✗/✓ **yok**, metin yalnız token: paper 16,17 · porcelain/85 10,65 · porcelain/70 7,62) · `<details>` SSS (10, script yok) · yeşil CTA + üstü çizili slogan (`<s>` yalnız CTA'da, `<h1>` dört satır, test pinli) · fiyat kartı + **4 gerçek** madde (panel+FLAGGED ✓ · CSV ✓ · manuel ✓ · lokasyon/departman ✓; API ✗ plaket ✗) · *"Security"* ≡ *"Four pieces of evidence"* **birleştirildi**, *"Proof of …"* etiketleri mevcut kartlara. · 🔴 **Taşınamayanlar — dosyadaki her cümle için sebep yazıldı ve render'da 100+ desenle tarandı, hepsi 0:** ölçülmemiş sayılar (*"under 10 minutes"*, *"30 seconds"*, *"two seconds"*, *"€90"*, *"pay period"*, *"ten seconds"*, *"A5"* → `MakesNoUnmeasuredNumericClaim`) · üstünlük hükümleri (*"Retire the fingerprint box"*, ✗/✓, *"goes blind"*, *"less than one terminal's maintenance"* → `ComparesMechanismsAndDeclaresNoWinner`) · **mount edilmemiş yetenek** (*"API for payroll"* — `/api/*` yalnız tap formu POST'ları · *"CSV import"* · *"live headcount"* · *"Book a pilot"* mailto · *"runs alongside"* · *"in view of your camera"*) · zamanlama iddiaları (*"instantly"*, *"that second"*) · **yanlış karar kuralı** (*"IP + GPS have to agree"* — satır 6 **AYRIK**) · plaket UID şekli (`91AC-7E55`) · Google Fonts `<link>` · `tappa-portal.html` · **marka**: 17 palet dışı hex · 12–24px yuvarlak kart/hap buton · emoji ikon · `backdrop-filter` · `infinite` · saffron **metin** · yeşil kaşe metni · `<script>` · yuvarlak nokta logo. · ✅ **Yazarken bulunan 4 düzeltme, biri base'de 4 denetimden geçmişti:** *"The ones we are actually asked"* (SSS başlığı; müşteri görüşmesi iddiası, pilot başlamadı — **base'deydi, 4 denetim kaçırmış**, yapıcı yakaladı) → *"Answered before you have to ask"* · *"with the manager's name on them"* **×3** (biri base'de) — ürün kimliği **saklıyor** (`entered_by`, `actor_id`) ama hiçbir ekran/CSV/fiş ismi **göstermiyor** (docket *"Entered by a manager"*, `ListPanelTransactions` `entered_by`'ı SELECT etmiyor) → *"recorded against the manager who entered it"* · `line` token'ı koyu zeminde **metin** olarak (10,62 geçiyordu ama marka: kenarlık) → `porcelain/70` · WIP sahnesinde **hareket açıkken eğim kayboluyordu** (`docket-print` `transform:none` + `fill-mode:both`; CDP ile ölçüldü) → eğim ayrı iç sarmalayıcıda, iki tercihde de 2°. · ✅ **Yeni `internal/handler/marketing_facts_test.go`** — `Fact`/`factDerivations` sözlüğü, **üründen türetilmiş** (policy sid+effect · `tap.Input`/`manual.Entry`/`ledger.Report` reflection · rota kayıtları · migration GRANT/FORCE RLS · `rolbypassrls`), anti-vacuity eşikleri, ve **3 yokluk tripwire'ı** (API yok · file import yok · headcount yok). Mevcut `marketing_test.go` + `marketing_claims_test.go` **bayt-aynı**. **38 test** (28+10). **Mutasyon: yapıcı 22/22 kırmızı · denetçi bağımsız 15 → 11 kırmızı, 4 yeşil**; yeşillerden **M11** (*"A position on its own is not enough"* ↔ *"is enough"* hayatta kaldı — QR kuralının yarısı pinsizdi) **kapatıldı**, M9/M10/M12 **kayıtlı sınır** (tripwire *yokluğu* okur, cümleyi değil — `landingview.go:311`'de yazılı). · 🔴 **5. TUR DENETİM: ONAY, 0 bloklayan** — bu sayfanın tarihinde **ilk kez** ilk denetimden geçti (önceki 4 turun 3'ü RED). Denetçi 47 yeni cümleyi ürüne karşı okudu, 10 SSS cevabını `policy`/`handler`/`db` ile 10/10 eşledi, referansı portla **yan yana** koydu: koyu plaket · basılan adisyon · yapışkan nav · koyu ledger · akordeon · yeşil CTA · üstü çizili slogan — **hepsi tanınır**. **3 bilinçli sapma, kullanıcıya gösterildi, tam sayfa ekran görüntüsüne "süper" dedi:** ledger *how it works*'ün ardında değil **merdivenin sonrasında** (kanıt→yargı→karşılaştırma) · fiyat tek kart değil iki · CTA ortalı değil iki sütun. **4 bloklamayan öneri uygulandı** (M11 pini · `marketingSection`'a 64px scroll margin — 390'da eyebrow sticky bar'ın **−13px** altındaydı, şimdi **+51** · *"manager's name"* ×3 · paylaşılan shell yorumu "three edits" → liste, **beş** değişiklik). · **Miras, yargılandı, kayıtlı sınır:** fiyat prose'unda *"Plaques are included, and so are replacements"* + CTA *"Taptime encodes each one…"* — **fiyat koşulu** (handoff §11), mount edilmiş yetenek değil, *"3 months free"* ile aynı sınıf; ve cümle **iki cümle sonra kendi sınırını yazıyor** (*"Until a plaque is mounted there is nothing to tap at"*). Kullanıcı isterse *"Plaques and replacements cost nothing extra"* daha net. · **Ölçümler (go1.26.6 = CI):** `make check` **0** · `make audit` **0** (govulncheck 0 · redline 0) · 38 test · `<svg>` 4 (`role=img`+`aria-labelledby`) · **0 mutlak URL · 0 `Tappa` · 0 `<script>` · 0 `style=` · 0 palet dışı hex** (diff'teki 27 hex'in hepsi `docs/design/` referans dosyasında) · `href="/signup"` 4 · **ölü CSS base'in altında** (7 → 6, `.relative` artık giyiliyor) · en düşük kullanılan kontrast **6,03** (ink/70 on paper), koyu zemin **7,62** · `tappa-green on ink` (2,09) **kullanılmıyor** · saffron/line **hiçbir yerde metin değil** · motion CDP: `docket-print`+`plaque-field` `iterations:1` `no-preference` içinde, reduce'ta 0 animasyon/opacity 1/eğim 2° · dokunma hedefleri 6 sayfa × 2 genişlik: 44 altı yalnız sr-only skip link · yatay taşma 1280/768/500/390 **yok** · sihirbaz 4 + legal 4 ekran 200, landing nav'ı **sızmadı**, sade header **61 → 77px** (44px floor'lar) · `input.css` **saf ekleme** (120/0), `.docket`/`.stamp`/`.btn` **değişmedi**. · ⚠️ **Süreç notları:** ajanlar bu Mac'te yerel süreç — **sistem uykusu/kapak/VSCode kapanışı öldürür** (bu oturumda 2 kez); uzun tur başlatırken `caffeinate -dims -t 14400` (bu turda koşuldu, işe yaradı) · 2026-09-12'de yapıcı yarıda durdurulunca *"hiçbir şeye dokunmamış"* denmişti — **yanlıştı**, bildirim yalnız ilk mesajı gösteriyordu, ~1 saatlik iş vardı; **her zaman `git status` ölç**. · **Backlog adayları (denetçi buldu, bu diff'in işi değil):** `signup` sayfasında header *Sign in* `href=""` (base'de de öyle) · SSS *"phones that cannot read the plaque"* cevabı sunucu için doğru ama basılı QR M8-05'e kadar yok (base, 4 denetim geçmiş). · **Ledger DEĞİŞMEDİ** — "ŞU AN" hâlâ M8-05 FAZ B3. **SIRADAKİ:** dalı push (CI koşar, deploy **koşmaz**) → `main`'e birleştirme **kullanıcı kararı** (= canlıya çıkar) · T72/T73 backlog'da)*

> *(19. oturum, 2026-09-09 — 🟢 **LANDING TAM YENİDEN TASARIM DONE** (`0b58bac` + `0300fb8`) · 🔴 **VE BİR KULLANICI KARARI DEĞİŞTİ: İŞ ARTIK FEATURE DALINDA.** Kullanıcı `landing-redesign` dalını istedi; CLAUDE.md §10'un *"doğrudan `main`'de çalışılır, görev/milestone başına dal açılmaz"* hükmü (2026-07-24 kullanıcı kararı) bu iş için **geçersiz**. Yan faydası ölçüldü: `ci.yml` `on: push` ile **her dalda** koşar, `deploy.yml` `workflow_run … branches: [main]` ile **yalnız main'de** — yani dal, CI doğrulaması verip üretim riski vermiyor. **PUSH EDİLMEDİ**, `main` `40bc88c`'de duruyor, üretim etkilenmedi. · **Kapsam:** 5 dosya — `landing.templ` · `landingview.go` · `landing_templ.go` · `input.css` (**saf ekleme**, 120/0 → panel ve tap ekranı bayt olarak etkilenmedi) · **yeni** `internal/handler/marketing_claims_test.go`. Migration YOK, yeni bağımlılık YOK, `internal/domain/**` ve `db/**` dokunulmadı. · **Teşhis ölçümle kondu:** eski sayfanın *güven* yarısı zaten güçlüydü (19 test kopyayı çiviliyordu) ama *teknoloji* yarısı görsel olarak **yoktu** — canlıda **0 `<svg>` / 0 `<img>`**, sekiz bölüm aynı kutuyu tekrarlıyordu. Çare: 4 inline SVG (**her biri bir mekanizma**: plaket · uçtan uca tap + **sayaç mandalı** · zincir · 24 saatlik şube), §5'in yedi satırını ürünün kendi kaşe damgalarıyla basan **karar merdiveni**, docket motifinin sayfaya yayılması, ve kanıt/yargının **fiyattan önce** gelmesi. · 🔴 **4 DENETİM TURU, 3'Ü RED — VE ÜÇ RED'İN ÜÇÜ DE AYNI SINIFTAN: BİR DÜZELTMENİN KOMŞU CÜMLEYİ YA DA KOMŞU KODU ÇÜRÜTMESİ.** Merdivenin **3. satırı ÜÇ KEZ YANLIŞ YAZILDI**: (1) *"ürünün kayıt yazmayan tek yolu"* — yanlış, `sys:tenant-mismatch` **iki tenant'ta da** yazmıyor (`guardrails.go:259`) + tap arayüzünde üç ret daha *"Nothing was recorded"* diyor, **ve sayfa kendi karşı örneğini iki paragraf sonra kendisi yazıyordu**; (2) düzeltmesi *"yabancı organizasyon telefonu **aynı şekilde** geri çevrilir"* — yanlış, `sys:no-session` **303 → /activate**, `sys:tenant-mismatch` sayfayı açtırıp **butonda 403** veriyor ve `tap.go:267` bunu birebir yazıyor (*"does not send them to activation, because their session is perfectly good"* — §4.5, mekân yabancı tenant'a açık edilmiyor); (3) onun düzeltmesinde *"davetiyesi **gösterilir**"* — yanlış, kodsuz `/activate` `problemNoLink` veriyor, yani davetiyeyi **istiyor**. ⚠️ **Ve (2) ile (3)'ü DENETİM DEĞİL, YAPICININ KENDİ DÜZELTMESİNİ YENİDEN OKUMASI BULDU** → düzeltme brief'lerine *"yazdığın her yeni cümleyi o gözle bir kez daha oku"* şartı eklendi ve işe yaradı. · 🔴 **İKİNCİ BLOKLAYAN: `make audit` TEMİZDEN FAIL'E DÖNMÜŞTÜ VE CI'YI KIRARDI.** Yeni testteki sentetik fixture (`UPDATE tags SET last_ctr = @ctr;`) **R4**'e takılıyordu — R4 satır-yereldir ve R3'ün aksine `*_test.go` muafiyeti **taşımaz**; `ci.yml` `make audit`'i `continue-on-error` **olmadan** koşuyor. ⚠️ **Orkestratör kaçırmıştı**: `make check` audit'i **içermiyor** ve ben yalnız onu koşmuştum. Çare **muafiyet değil literal bölme** (muafiyet tarayıcıyı kalıcı körleştirir); düzeltirken **yapıcının kendi açıklama yorumu ikinci bir R4 ihlali üretti** — R4 yorum nedir bilmez. · 🔴 **KÖK SEBEP BULUNDU VE KAPATILDI: 7 iddia-taşıyan yapının 5'i KORUMASIZDI** (yalnız `LandingComparison` + `LandingAudiences` pinliydi) — **1. RED tam o boşluktan sevk edilmişti**. 9 yeni test **ayrı dosyada** (mevcut 19 test **bayt olarak** dokunulmadı): `LandingRules` + `LandingTapFlow` **iki** invariant'la (render + **ürün çapası**), diğer üçü render invariant'ıyla (tiplerinde `Sources` yok — **kayıtlı sınır**). Çapalar **kopyadan değil üründen** türetiliyor: `policy.Guardrails`/`Baseline` (sid **ve** effect), `sun` reflection, `AdvanceTagCounter` gövdesi, `checkin.OutcomeForeignTenant` kolu. Merdiven ayrıca satırların **`internal/policy`'nin kendi değerlendirme sırasında** basıldığını doğruluyor. **Mutasyon: yapıcı 13, denetçi bağımsız 8 → 21/21 kırmızı**; anti-kandırma mutasyonu (tokenleri **yoruma taşı**) da kırmızı. ⚠️ **Mutasyon testi yapıcının KENDİ testinde iki kusur buldurdu**: tarayıcı SQL **yorumunu** kod sanıyordu (`<` → `<=` mutasyonu **yeşil geçiyordu**) ve gövdeyi ilk `;`'de kesmek yorumun içinde bitiyordu. · 🔴 **VE BİR SESSİZ KUSUR: ÜRETİLEN DOSYA BAYATTI.** `make gen` koşulduğunda `landing_templ.go`'nun **N4/N5/N6 düzeltmelerini hiç taşımadığı** görüldü — `.templ` kaynağında vardılar, **render eden Go'da yoktular**; öyle sevk edilse üç düzeltme de sessizce çalışmayacaktı (CLAUDE.md §2'nin `make gen` şartının maliyeti **ölçüldü**). · ✅ **Ölçümler (go1.26.6 = `ci.yml`'nin sabitlediği sürüm):** `make check` **exit 0** / 25 paket · `make audit` **exit 0** (govulncheck 0 · redline 0, base ile `[R…]` satırları **birebir aynı**) · **28** landing/marketing testi PASS · render: **4 `<svg>`** · **0 mutlak URL** · **0 kullanıcı-görünür `Tappa`** · CTA `/signup` ×3 · **ölü CSS base'in ALTINDA (7 → 6)** — `.ring` base'de de varmış · kontrast 9 oran bağımsız yeniden hesaplandı, en düşük kullanılan **5,69:1** · `saffron`/`line` **hiçbir yerde metin değil** · animasyonlar `prefers-reduced-motion: no-preference` içinde · 18 tablonun **18'inde** RLS. ⚠️ **`go.mod`'un asgari sürümü `go 1.26.2` bu makinede kendi stdlib'ini DERLEMİYOR** (`crypto/internal/fips140/drbg` çift tanım; iki ajan bağımsız ölçtü) — CI **1.26.6** kullandığı için üretim etkilenmiyor, ama sürüm değiştirmek `cmd/rotatekek` ve `staticcheck`'te **sahte kırmızı** üretiyor. · ✅ **YAN İŞ — İSİM İNCELEMESİ, KARAR: `tappa` İÇ KOD ADI OLARAK KALIYOR** (kullanıcı kararı). Ölçüldü: **5.396 tekrar / 491 dosya**. Üçü **isim değil çalışan değer** ve dokunulmamalı: **kripto domain-separation etiketleri** (`tappa/tap-context/v1|` vb. — HMAC anahtar türetme girdisi; değişirse **her canlı oturum, davet kodu, sıfırlama linki** geçersiz) · **`tappa.app` policy URN'leri** (`uuid.NewSHA1` ile `policies`/`policy_versions` **PK'sini** türetiyor; değişirse üretimdeki baseline satırları **öksüz** kalır, bir sonraki tap **kopyalarını** yazar) · **`tappa_*` DB rolleri** (1.542 tekrar, canlı, ve `redline-check.sh` rol adını **tam eşleşmeyle** denetliyor). Ayrıca **10 çerez adı** `tappa_` taşıyor (tarayıcıda görünür ama değiştirmek herkesi çıkış yaptırır). · **Deploy: ayar GEREKMEDİ**, zaten kurulu. · 🔴 **BACKLOG (bu oturumda açıldı, ikisi de kapanmadı):** **(a)** panel tenant yöneticisine debounce için *"The window is yours to set"* diyor (`internal/domain/tenant/rulebook.go:806`) — landing'den attığımız iddianın **aynısı**, orada duruyor ve **yanlış** · **(b)** `make audit`'te **marka kapısı YOK**: kullanıcı-yüzü metinde `Tappa` sızıntısını mekanik olarak tarayan hiçbir şey yok, bu turda sızıntı olmaması **elle** doğrulandı. · **SIRADAKİ: ledger DEĞİŞMEDİ** — "ŞU AN" hâlâ **M8-05 FAZ B3** (donanıma bloke); landing plansız bir kullanıcı işiydi. Kalan: dalı push (CI doğrulaması, deploy **yok**) ve `main`'e birleştirme **kullanıcı kararı**)*

> *(18. oturum, 2026-09-02 — 🟢 **DOMAIN CUTOVER: `tappa.everva.com.tr` → `taptime.mt`** (kullanıcı isteği, CF kimlik bilgileri verildi) · **Cloudflare DNS CANLI**: apex `A` + `www CNAME` → `144.76.158.60`, ikisi de **DNS-only / proxy KAPALI** — §4 proof-of-place invariant'ı gereği (proxy açık = `$remote_addr` CF edge IP = §5 IP kanıtı çöker + immutable `transactions`'a sahte "network proof of place"); `dig +short taptime.mt` → `144.76.158.60` doğrulandı · **kod/config/CI/docs güncellendi**: `40-ingress.yaml` — **apex TEK serving host**, yeni `tappa-redirects` Ingress'i eski domain **ve** `www`'yi apex'e **301**'liyor (redirect-only, tap işlemez; eski plaket/link 404 vermez, §4.3) · `05-config.yaml` `TAPPA_BASE_URL=https://taptime.mt` · proxy-off gate (`deploy.yml:470` + `verify-deployment.sh:52`) artık **taptime.mt**'yi denetliyor · `deploy/README.md` + `m8-deploy-pilot.md` + **Q08 KAPANDI** (domain alındı) · cookie-shadowing yorumları (`cookies.go`/`session/cookie.go`) — cutover, apex'e sahip olarak sibling-shadowing önkoşulunu **kapattı** · **`tappa-security-auditor` ONAY**: sömürülebilir kritik/yüksek ihlal yok, en kritik invariant doğru host'a taşınmış; 3 iyileştirme uygulandı (**www redirect-only**, ingress'ten **hesap e-postası kaldırıldı**, cookie yorumları düzeltildi) · ⚠️ **açık uç**: `tappa-redirects` ACME renewal'ı (~60 gün) ve redirect'in path koruması ilk deploy'da **doğrulanmalı** (yorumda komutlar) · ✅ **PUSH + CUTOVER CANLI (2026-09-05, kullanıcı `deploy alamadın mı` deyince push edildi).** Push CI'yı kırdı — ama **domain'le ilgisiz, ay-dönümünde patlayan ÖNCEDEN VAR OLAN** iki sorun yüzünden (son yeşil CI 2026-08-25'teydi, arada CI koşmadı): **(1)** 4 billing tarih-bombası testi (July sabitliyor, kod "geçen ay"ı render ediyor; Ağu→Eyl'de kırıldı) → `e4f5847`, `?month=` ile deterministik, üretim kodu dokunulmadı · **(2)** `tap.decision` intermittent §4.6 panic-flake (armed logger'ın tek panic'i `-race` full-paket yükünde recover zincirini ara sıra aşıp binary'yi çökertiyor; tek başına/subset yeşil, CI'da 2/2 kırmızı; koruduğu koşul üretimde ULAŞILAMAZ, garanti `TestSecurityAlert_TheDecisionLineComesAfterTheEvidence` ile yapısal pinli) → `79a1deb`, **T70 KARANTİNA** (`t.Skip` unless `TAPPA_RUN_FLAKY=1`, silinmedi). Ayrıca **DNS'i önceden oluşturmamın** açtığı iki gerçek deploy-tooling hatası: **(3)** proxy-off preflight origin cert'ini doğruluyordu → çıkmamış-cert'te self-signed fail → `58ac9e2` (`curl -k`; gate'in işi cf-ray tespiti, cert değil) · **(4)** redirect `$request_uri` admission webhook'una takıldı → `cc91ee2` (kaldırıldı). **Deploy `33962926816` başarılı.** 🟢 **CANLI DOĞRULANDI:** `taptime.mt`+`www` → `144.76.158.60` **DNS-only, cf-ray YOK** (proxy kapalı, §4 invariant), **geçerli Let's Encrypt cert** (`ssl_verify_result=0`), `/healthz`+`/readyz` **200** (DB'ye ulaşıyor), HSTS, eski domain+www **301 → apex**. ⚠️ **UYARI: 301 path'i DÜŞÜRÜYOR** (`/activate?x=1` → `Location: https://taptime.mt`; ingress-nginx permanent-redirect URI eklemiyor + `$request_uri` admission-block + snippet küme-çapında kapalı) → eski derin linkler apex ana sayfaya düşer; **pratik etki ~sıfır** (Q08 tam bu cutover'da kapandı, eski domainli kesinleşmiş üretim plaketi/linki yok), **T71** kayıtlı (app-seviyesi redirect ya da snippet — kullanıcı kararı). ⚠️ **API token sohbette düz metin ifşa oldu → kullanıcıya ROTATE önerildi.** ⚠️ Açık uçlar: `tappa-redirects` ACME renewal (~60 gün, yorumda probe) · T65 GPS-flake CI'da hâlâ latent. **SIRADAKİ: milestone ledger DEĞİŞMEDİ** — M8 pilot kapıları/kullanıcı kararı hâlâ açık · ✅ **REBRAND CANLI (2026-09-07): kullanıcı-yüzü `Tappa` → `Taptime`** (`0738d16`, 71 dosya, wordmark `taptime`) — kapsam **yalnız kullanıcı-yüzü** (kullanıcı kararı): şablonlar/başlıklar/prose/wordmark, legal/marketing, CSV/PDF başlıkları, indirme dosya adları (`taptime-hours.csv`/`taptime-billing.csv`), policy etiketleri (`Taptime guarantees/baseline`), `AuthorSystem`, + assert eden testler. 🔴 **Teknik tanımlayıcılar dokunulmadı ve net-0 diff ile denetlendi:** modül yolu · `TAPPA_*` · `tappa_*` DB rolleri · k8s/Docker · `tappa-green` Tailwind token'ı · **kripto `tappa/tap-context` domain-separation etiketleri** · `tappa.app` policy URN · `X-Tappa` header · `cmd/tappa`. Kod yorumlarındaki iç kod adı `Tappa` **bilinçli kaldı** (kapsam dışı; birkaçı artık render metniyle uyuşmuyor — istenirse ayrı yorum/doküman taraması). Canlı doğrulandı: `taptime.mt` landing wordmark `taptime`, `<title>Taptime — …`, kullanıcı-yüzü `Tappa` sızıntısı **yok** (kalan 7 `tappa` token'ı yalnız `tappa-green` class'ı). `make gen` + `make check` + `go test -race ./...` yeşil)*

> *(17. oturum, üçüncü yarı — 🟢 **PUSH + DEPLOY YAPILDI: CANLI `af8b44c`, `goose … version: 22`, §4.5 kapısı geçti** — yani **encode uç noktası ve `internal/sun`'ın wipe disiplini ÜRETİMDE** · 🔴 **AMA PUSH BİR ŞEY ORTAYA ÇIKARDI: main 2026-08-22'DEN BERİ KIRMIZIYDI**, aynı testle (bugün `…CanSeekOnTheIndex` olarak yeniden adlandırıldı), yani **üç gündür hiçbir şey deploy edilmiyordu ve fark edilmemişti** · kusur **testindi, planlayıcının değil**: test indeksin **tercih edilmesini** bekliyordu, CI'da tablo küçük olduğu için **Seq Scan ucuz** ve planlayıcı **doğru** olanı yapıyordu; yerelde yeşildi çünkü dev DB **~279.000 satır** · 🔴 **testin *"5 satır yeter"* ölçümü bir ARTEFAKTMIŞ — geri alınan sondalar bile ölü heap sayfaları bırakıyor, süpürme tabloyu KENDİ AYAĞININ ALTINDA şişiriyor**; gerçek geçiş **~250 satır**, fixture **%20 altında** · **(A) elendi: eşik `random_page_cost` 1.1↔40 arasında 25 KAT kayıyor, testin seçebileceği bir sayı değil** · ✅ çare yine **soruyu değiştirmek** oldu (*"seçecek mi"* → *"kullanabilir mi"*), ve **yapıcı benim önerimi ölçümle çürüttü**: ANALYZE olmadan planlayıcı **yanlış indeksi** seçiyor · 🔴 **daralma dürüstçe yazıldı ve backlog T17'ye kaydedildi: T17'nin asıl korkusunu artık HİÇBİR KAPI korumuyor** · **SIRADAKİ: ledger'da bloke olmayan görev yok; hijyende T65 (*"M8 pilot öncesi"*)**)* — *(ikinci yarı: ✅ **BACKLOG T64 DONE** (`f540f8a`): `internal/sun/cmac.go` **hiçbir ara tamponu silmiyordu** ve bu **SUN doğrulaması dahil her MAC yolundaydı**; **34 tampon** deferred-wiped, kapı **üç dosyada 146 bildirimi** sınıflandırıyor · 🔴 **en ağırı CBC biriktiricisiydi — dönüşte `KSesAuthENC`/`KSesAuthMAC`'in birebir kendisi, ve HEAP'te** · 🔴 **bir muafiyet gerekçesi tersini söylüyordu:** `mac` ve `want` **karşılaştırma** değerleridir, ve **reddedilen** bir tap'te sayaç ilerlemediği için kalan `mac` saldırganın **sahip olmadığı** doğru MAC · 🔴 **kapı DÖRT KEZ yenildi** (`x[:0]` · ulaşılmayan `defer` · `else if`'e kör yürüyüş · **kardeş bloklar**) ve çare bir **ÖLÇÜT** oldu: *"kazayla yazılanı KAPAT, kasıtlı kaçamağı SAY — bir kapı bir güvenlik sınırı değildir"* · 🔴 **güvenlik merceği yine doğruluk merceğinin göremediğini buldu:** `cipher.NewGCM` `Block`'u **değerle** gömüyor → **her tap ~1 KB referanssız heap bırakıyor, içinde parkın tüm tag anahtarlarını açan KEK** — **kapatılamaz, SAYILDI** · ⚠️ **KAT'lar bu sınıfa KÖR (ölçüldü)** · ⚠️ **iki süreç kusuru: benim mührüm izlenmeyen dosyayı görmüyordu; yapıcının harness'ı üç kez artık bıraktı** · 🔴 **VE LEDGER'DA BLOKE OLMAYAN GÖREV KALMADI** — B3 (Android zinciri **bu makinede yok** + donanım + md.5 + Q08) · M8-06 (Q13+T45) · M8-07 · M9 (MVP dışı/Q22) · **SIRADAKİ: kullanıcı kararı**)* — *(önceki yarı: 🔴 **M8-05 FAZ B2c-2b DONE: ENCODE UÇ NOKTASI** (`f632f09`) · üç rota `ProtectWriting` + encode bütçesinin **iç grubunda**, **tenant ve aktör OTURUMDAN** (denetçiler **on beş taşıyıcı** denedi, hiçbiri geçmedi), `internal/encode` **artık üretimden çağrılıyor** · ✅ **ADR 0017 §6 md.10 · md.12 · devir md.18 KAPANDI** · 🔴 **11 yapıcı turu / 11 bağımsız denetim (9 üçüncü göz + 2 güvenlik), 10'u RED** · 🔴 **GÖVDE KAPISI ÜÇ TASARIM KAYBETTİ VE SONRA SAYILDI** (`body any` → anonim struct · mühürlü arayüz → **struct GÖMMESİ** · değer parametresinin kaldırılması → **muaf yazıcının `w` yetkisi**); üçünde de sızıntı **gerçek router'dan** ölçüldü ve **paket yeşil kaldı** — sınıf B2c-2a'nın §4.7 duvarıyla **aynı**, *"bir sızıntı nasıl görünür"* sorusu **kompozisyonla sonsuz**; **dördüncüsü denenmedi** · 🔴 **VE 2. GÜVENLİK GEÇİŞİ, BEŞ GENEL DENETÇİNİN GÖREMEDİĞİ GERÇEK BİR ÜRÜN KUSURU BULDU:** `plaque.unmarked`, kodun **kendisinin *"OLAĞAN"* dediği** tetikleyicide (telefon kapanır → istek ctx'i ölür) **hiç yazılmıyordu**; çare **iki katmanlı** oldu — olağan kapanma artık **ONARILIYOR** (`markEncoded` de kopuk context'te), telafi girdisi ise **DB gerçekten erişilemezken** yazılıyor · 🔴 **VE ASIL DERS: BİR SAYIM ON BİR TURDA ON BİR KEZ EKSİK ÇIKTI**, iki kez **düzeltmenin kendisi** yenisini üretti; çare liste değil **ÖZELLİK** oldu (*"telafi rapor ettiği yolu kat eder"* → **satır varsa DB erişilebilirdi, satır yoksa hiçbir şey kanıtlanmaz**) · `audit_log.actor_id` artık **gerçek admin id'si** (md.8'in kararı değişti, ADR'de **tarihsel damgayla**) · **SIRADAKİ: M8-05 FAZ B3 — Android rölesi, M8'İN TEK DONANIMA BLOKE FAZI**; devir listesi **28 madde, 5 kapandı / 23 açık**, ve **md.16 hepsini yönetiyor: hiçbir çip encode edilmedi**)*)
...
| | |
|---|---|
| **🟢 GÜNCEL (27. oturum, 2026-09-26) — aşağıdaki satırlar tarihsel** | ✅ **M10 A1 OP-5 DONE** (`6841d68`, dal **`m10-a1`**, main'in önünde): migration **00026** — `platform_admins`, `platform_sessions`, `operator_audit_log` (append-only), `operator_read_tickets`; roller `tappa_operator` + `tappa_opdefiner` **`scripts/db-init/01-roles.sql`'de** (migration'da değil — R5b); beş `op_*` definer (`op_touch_session`, `op_record_auth_event`, `op_open_session`, `op_complete_enrollment`, `op_close_session`); 00026 roller yoksa/üyelik taşıyorsa **SQLSTATE 55000** ile düşer ve runbook'u adıyla gösterir. 🔴 **DEPLOY ÖN KOŞULU:** main'e birleştirmeden **ÖNCE** canlı kümede `deploy/README.md` → *"Operator roles (M10 OP-5)"* runbook'u (orkestratör koşar, `--context hetzner-k8s-1 -n tappa`, sır içermez; 2. adımın beklenen çıktısı belgede). Roller olmadan birleştirilirse migrate Job 3 kez düşer, "Roll out" koşmaz, **eski pod servis vermeye devam eder**, şema 25'te kalır (manifestlerden ölçüldü) → runbook + workflow_dispatch. `tappa_operator` LOGIN + parolası **OP-7**'de (kullanıcı: `tappa-secrets`'a dört ad). **Sıradaki: OP-6** — `internal/operatorauth` (parola, TOTP, oturum, çerez, token, limiter/kilit) + `internal/sun`'a `Seal`/`Open`; kabuller m10-platform.md *"Kabullere bağlananlar — OP-6"* + OP-5 blokları (madde 1–36; kilit N=5 / 15 dk **geçici**, nihai OP-6/OP-8). OP-7'ye adıyla devredilenler kartta (a)–(d): havuz `log_parameter_max_length_on_error=0`'ı başlangıç parametresi olarak iğneler + açılışta ≠0 ise ret · yalnız bağlı parametre · `PgError.Detail` log'lanmaz · `operator.sql`/`operator.go`. CLAUDE.md §4.5/§6/§7 güncellendi; **§3 dizin haritası** OP-6/7/9 ile. *(Aşağıdaki OP-4 cümlesi bir önceki adımdır.)* ✅ **M10 A1 OP-4 DONE** (`b0b34c0`, dal **`m10-a1`**): ADR **0020** (platform operatörü: ayrı kimlik, zorunlu TOTP, ayrı oturum/çerez/host) + ADR **0021** (`op_*` SECURITY DEFINER: tenant-ötesi erişimin tek yolu — iki aşamalı okuma, `clock_timestamp()` kuralı, yazma dönüş pini) + ADR 0016 "kısmen yerine geçildi". **Sıradaki: OP-5** — migration **00026**: `platform_admins`, `platform_sessions`, `operator_audit_log`, bilet tablosu, roller (`tappa_operator`, `tappa_opdefiner`) + OP-5'te doğan beş definer (`op_touch_session`, `op_record_auth_event`, `op_open_session`, `op_complete_enrollment`, `op_close_session`) + katalog/davranış testleri (kart listesi m10-platform.md OP-4 bloğunda). **Açık iş:** CLAUDE.md güncellemesi (§3 dizin haritası, §4.5 cümlesi, §7 "asla loglanmaz"a bilet/TOTP kodu/enrollment token) — OP-5 ya da OP-10 ile. *(Aşağıdaki deploy cümlesi Faz 0'ın kapanışıdır.)* ✅ **DEPLOY EDİLDİ (kullanıcı kararı, 2026-09-26): `main` = `478f93e`, canlı imaj `sha-478f93e`, üretim goose sürümü 25, healthz/readyz 200; 00025 trigger'ı yerinde, `tenants` ACL `tappa_app=r`.** Main CI yeşile döndü (`b09ba1e`'den beri kırmızıydı). **Sıradaki: M10 A1 — OP-4 (ADR 0020 operatör kimliği + ADR 0021 `op_*` arayüzü), yeni dalda.** Aşağıdaki cümleler birleştirme öncesi yazıldı. **M10 — [Platform](m10-platform.md) FAZ 0'IN KOD KISMI BİTTİ**, dal **`m10-faz0`** (origin'de; `main` = `b09ba1e`, canlı `aefa67d` içeriği). ✅ F0-5 `ea43e4d` + kesinti düzeltmesi `71e7ef8` · F0-7 `3844ea1` · OP-2 `30284ce` · WL-5 atıf düzeltmesi `e226eea` (main CI'ı yeşile döndürür) · F0-6 `01ec1c8` + `64f7618` · OP-3 (migration **00024**) `79d9f84` · F0-6b (migration **00025**) `0f3979a` · belgeler `a488334`, `7907a73` ve bu commit. Her görev: üçüncü göz ONAY + tappa-security-auditor ONAY. ⏳ **Sıradaki: MAIN'E BİRLEŞTİRME = DEPLOY = KULLANICI KARARI.** Deploy üretimde **00024 + 00025**'i koşar (üretim ACL'i ve `tags` zemini ölçüldü, dev'le aynı); Rusty Bar plaketinin kartı "This plaque needs replacing" gösterecek. Birleştirmeden önce üretimde `tags` ve `tenants` zemini yeniden ölçülür. Sonra A1 (OP-4: ADR **0020** + **0021**); sıradaki migration **00026**. **Kullanıcıda:** F0-1 panel parolası rotate · F0-4 DB parolaları rotate · Rusty Bar plaketi Replace · SES dış adımları · **yedekleme kurulumu (backlog T45 — 2026-09-26'da ölçüldü: kümede backup CronJob YOK; bir çip bu yüzden kalıcı kayboldu)** · dev DB'de kalan `tappa_f06aud1` veritabanı (silme komutu sohbette). **Sır kapısı:** push öncesi kanca bu makinede kurulu (`make hooks`); push daima uzağın ADIYLA (`git push -u origin <dal>`). |
...
| M8-05 B3a | Encode aracı — **Android APDU rölesi** (kod yarısı) | **done** | `4fc5e3f` · **3 yapıcı turu / 2 bağımsız denetim (1 üçüncü göz + 1 güvenlik), 1 RED** · Kotlin, `android/` (Go modülünün dışında, `make android` / `make android-test`), **tek runtime bağımlılık `kotlin-stdlib`** · giriş **WebView + `CookieManager`** (HTML kazıma yok, parola uygulamada tutulmaz) · `IsoDep` reader mode, tur reader thread'inde, **on alışveriş** · 🔴 **`ContractPinTest` Go kaynağını okuyup üç rotayı, yedi fault'u, on adım adını, iki form alanını ve çerez adını çiviliyor** — ve **ilk sürümü sıcak ağaçta KÖRDÜ** (Go dosyaları Gradle girdisi değildi; altı sapma `UP-TO-DATE` ile yeşil geçti) → girdi bildirildi, 6/6 kırmızı · **§4.7 sayımı:** telefon **anahtar tutmaz** ama **sırsız değildir** — tur boyunca `authenticate`+`changekey` gövdeleri ADR 0017 §3'ün *"döküm altında anahtara eşdeğer"* dediği şey, Kotlin sıfırlayamaz, debug APK'da `am dumpheap` **root'suz** çalışıyor (güvenlik denetçisi ölçtü, README sayıyor) · **33 JVM testi, 17 mutasyon, 0 hayatta kalan** · **Galaxy A15'e kuruldu ve açıldı, FATAL 0** · ⚠️ **release imzası/hedefi YOK** — debug APK operatöre verilecek artefakt değil |
| M8-05 B3b | Encode aracı — **fiziksel doğrulama** (silikon yarısı) | **done** | 🎉 **2026-09-19: GERÇEK ÇİP ENCODE EDİLDİ VE UÇTAN UCA TAP DOĞRULANDI.** İki silikon düzeltmesi: `fd1b667` (plain WriteData — **md. 7 cevaplandı:** fabrika NDEF dosyası CommMode.**Plain** ister, 917E buydu) · `4dd6959` (CmdCtr — **gerçek çip plain WriteData'yı SAYIYOR**, §9.1.2 harfinin aksine; 911E buydu). Üç adımda encode: 917E → plain fix → 911E → ctr fix → **on step 200, `encoded_at` damgalı**. **md. 8 (authenticated key 0 sonrası oturum) HÂLÂ AÇIK** — bu tur key `0x01`'e ChangeKey yaptı (oturum yaşadı, changefilesettings geçti); authenticated key 0 senaryosu §5.1 adım 8'e (md. 5) bloke, ölçülmedi. **Tap uçtan uca:** çip→mount+active, GPS+IP, personel+davet+aktivasyon, tap → `sun_valid=true` (ctr 6→10), tap motoru her §5 kararını verdi (practice·debounce·gps-conflict·**ip-or-gps-ok** `verdict=ok`). ⚠️ **md. 5 (anahtar 0 şeması) hâlâ AÇIK** — çip fabrika key 0'da, **duvara çıkamaz**, laboratuvar plaketi. Kalan runbook maddeleri (MSB-first encode doğrulaması, replay, Q11, baskı provası) B3b'nin bu turunda kapsanmadı. *(Aşağısı tarihsel — B3b'nin planıydı.)* 🔴 **DONANIM SENDE, KOD HAZIR: telefonu tut, plaketi tut.** Galaxy A15 `adb`'ye bağlı, APK kurulu · ilk gerçek tur **md. 16'yı** kapatır (*"hiçbir çip encode edilmedi"*) · silikonun cevaplayacağı üç açık soru: **md. 7** (adım 5 `WriteData` CommMode — Full mü Plain mi; yanlışsa `writedata`'da `refused`, anahtar değişmemiş, **kurtarılabilir**) · **md. 8** (`ChangeKey` sonrası oturum yaşıyor mu — sahte çip *"evet"*, `changekey.go` **tersini varsayıyor**; düşerse `changefilesettings`'te kırmızı, **anahtar değişmiş, SDM kapalı, satır yazılmış** → §5.3 kurtarma, **sevk edilmedi**) · **md. 9/12** (Tablo 65 `AEh`) · ⚠️ **İLK TUR ÜRETİME YAZAR:** `tags`'e silinemez bir satır (md. 12), o plaketin anahtarı üretim KEK'iyle sarılı, `encoded_at` damgalı — **laboratuvar plaketi olur, duvara çıkamaz** (md. 5: anahtar 0 fabrika varsayılanında kalıyor, §5.1 adım 8 sevk edilmedi) · **donanıma bloke** · `android/` Go modülünün **dışında**, ayrı `make` hedefi · 9 maddelik devir listesi runbook'ta: gerçek çiple uçtan uca · encode tarafının MSB-first yazdığının doğrulanması (M2-08'in kapatamadığı yarı) · replay denemesi · **Q11** (iPhone Safari çerez ömrü) · fabrika varsayılanlarının değiştiği · yazma izninin kilitlendiği · baskı provası |
...
### 2026-09-26 (27. oturum, dördüncü yarı) — ✅ **M10 A1 OP-5 DONE: migration 00026 + roller + beş `op_*`** (`6841d68`, dal `m10-a1`) · 5 yapıcı turu · üçüncü göz RED → ONAY · tappa-security-auditor ONAY · kapanış kontrolü ONAY

- **Ne:** dört tenant kapsamsız tablo (uuid PK, dizi yok, `tappa_app`'tan açık `REVOKE ALL`, ENABLE + FORCE RLS, R5 muafiyeti görünür) · `operator_audit_log` append-only (satır + TRUNCATE trigger'ı, zaman sütunu `clock_timestamp()` ve kimsenin INSERT listesinde değil) · roller `01-roles.sql`'de işaretli blokta, runbook bloğu `sed` ile keser · beş definer: `search_path = pg_catalog, pg_temp` + nitelikli adlar, EXECUTE yalnız `tappa_operator`, her zaman `clock_timestamp()`, dönüş `void` (touch: çağıranın kendi id'leri), TOTP adımı duvar saatine bağlı (cur±1), kilit koşulu aynı `UPDATE`'te, enrollment tek kullanım **iki bağımsız katmanda** (CHECK + fonksiyon yüklemi), constraint ihlali aynı 28000 reddine döner. `pg-restore-verify.sh` ve `redline-check.sh` yedinci append-only tabloyu adıyla tanıyor; türetme testi migration'lardan.
- **Turlar — her biri ölçülmüş bir açık:** (1) üçüncü göz RED — **B1** kullanılmış enrollment token'ının reddini tutan satır pinsizdi: silinince (M6a) reset-mfa şekilli `pending` hesap ESKİ token'la yeniden kuruluyordu ve 20 test yeşildi; kartın *"status tek başına yeter"* cümlesi yanlıştı · **O1** constraint DETAIL'i digest/zarf/e-posta/hash'i **değerleriyle** hataya taşıyordu (üretimde `log_min_messages=warning` → pod log'u) + *"koşullar tuttu"* kehaneti · **O2** close'un oturum yüklemi pinsiz · **O3** geri yükleme doğrulaması yedinci append-only tabloyu görmüyordu · **O4** runbook `--context`'siz. (2) ONAY + 6 düşük (ön koşul üyelik yönleri, `opdefiner` tenant yetki taraması, trigger yazım kör noktası…). (3) **güvenlik ONAY + 1 ORTA:** `log_parameter_max_length_on_error` **user-context** — DSN sahibi `ALTER ROLE tappa_operator SET … = -1` ile rol varsayılanı yazabiliyor (ölçüldü), parola rotasyonundan sağ çıkar ve reddedilen her çağrının bağlı parametreleri (ham enrollment token dahil) pod log'una düşer → OP-7'ye başlangıç parametresi iğnesi + bugün ters katalog pini (`pg_db_role_setting` op rolleri için 0) + runbook doğrulaması. (4) kapanış ONAY + 2 metin.
- **Yapıcının haklı çıkan itirazları (ölçümle):** `op_touch_session` audit **yazmaz** (kart *"hepsi"* diyordu; yazsa her kabul edilen işlemin arkasında iki satır olurdu) · DB tarafına ikinci tavan **eklenmedi** (ölçüldü: kimliksiz bir sel TOTP kilidini kapatıyordu) · dört tabloda **FORCE** RLS (`pg-restore-verify.sh` ENABLE≠FORCE'u reddediyor — yalnız ENABLE her felaket kurtarmayı *"hizmete alma"*ya düşürürdü) · hedef sütun `target_tenant_id` (`tenant_id` adı R5 ve RLS testlerini yanıltırdı).
- **Orkestratör ölçümü (üretim, salt-okunur):** `log_statement=none` · `log_min_duration_statement=-1` · `log_parameter_max_length=-1` · `log_parameter_max_length_on_error=0` · `log_min_error_statement=error` · `log_min_messages=warning` · `log_error_verbosity=default` → bağlı parametreler bugün pod log'una gitmiyor; iki koşul ADR 0021'de adlandırıldı. Dev `log_statement=all` (parametreler dahil) — dev sunucu log'u konsola basılmaz.
- **Sayılı yeni sınırlar:** 14 (SAVEPOINT ve `client_min_messages=log` kanalıyla TOTP adımı / *"koşullar tuttu"* kehaneti; yeni güç değil) · 15 (herkese açık `pg_stat_*` görünümleri gizli `pending`/`disabled` hesabın varlığını ve toplam hesap sayısını veriyor — definer tarafında kapatılamaz). Kilit şekli bilinçli: sayaç yalnız başarıda sıfırlanır (ilk kilitten sonra 96 tahmin/gün, sıfırlasa 480).
- **Ölçümler:** operatör testleri `-race` 48/0/0 · tam `go test -race ./...` yalnız bilinen T72 (`cmd/rotatekek`) FAIL + T70 SKIP · `redline-check` 0 FAIL · `make gen` idempotent · `go.mod`/`go.sum`/`sqlc.yaml` değişmedi · dev goose 26. Dev DB kalıntısı: eşzamanlılık testi her koşuda 1 hesap/oturum/audit satırı bırakır (append-only, tasarım gereği, kart madde 17). `tappa_f06aud1` dev veritabanı **silinmiş** (ölçüldü) — kullanıcı borcundan düştü.
- **CLAUDE.md:** §4.5'e `op_*` bilinçli tek istisna, §6'ya tenant kapsamsız tablo kuralı, §7 *"asla loglanmaz"*a okuma bileti / TOTP kodu ve sırrı / enrollment token'ı. §3 dizin haritası OP-6/7/9 ile.
- **Backlog:** **T78** (`adminauth` zamanlama testi tam-paket yükünde bir kez kırmızı) · **T79** (`tappa_app` aynı log ayarını kendi rol varsayılanı olarak çevirebiliyor — OP-7 ile birlikte).
- **Kapsam dışı bırakılan:** `deploy/README.md`'deki eski ~138 `kubectl` satırı `--context`'siz; orkestratör komutları her zaman bağlamla koşar.

### 2026-09-26 (27. oturum, üçüncü yarı) — ✅ **M10 A1 OP-4 DONE: ADR 0020 + ADR 0021** (`b0b34c0`, dal `m10-a1`) · 4 tur · güvenlik RED ×2 (1. ve 2. tur), sonra ONAY + kapanış kontrolü ONAY

- **Kullanıcı sorusu (aynı gün):** *"elimde tag bitiyor, hep yakıyoruz, tekrar kullanılabilir olsun"* — ölçüldü: üretimde yanmış **tek** çip var ve onu yakan encode değil, 26. oturumun çift-tenant temizliğinde **plaket satırının (anahtarın) silinmesi**ydi; kümede yedekleme CronJob'ı **kurulu değil**. Kullanıcı kararı: **plan değişmez**; geçici yol kod gerektirmez (encode edilmiş çip aynı işletmede Unmount → Mount ile taşınır); kalıcı çözüm backlog **T77** (yarım encode'u tamamlama · fabrika sıfırlaması · çip teşhisi); kural agent-brief + hafıza: **plaket satırı hiçbir temizlikte silinmez**.
- **OP-4 turları — her biri bir ölçümle kırıldı:** (1) güvenlik RED: okuma audit'i çağıranın transaction'ında → `SAVEPOINT`/`ROLLBACK TO` ile **iz bırakmayan tenant-ötesi okuma** → iki aşamalı okuma (audit önce commit edilir, veri bilete bağlı). Orkestratörün `xmin` önerisini yazar **ölçümle reddetti** (savepoint'te yaratılan bilet geçiyordu) → `created_xact xid8` + `pg_xact_status = 'committed'`. (2) güvenlik RED: bilet süresinin saati yazılmamıştı; deponun `now()` deyimi açık transaction'da **donuyor** → süresi dolmuş bilet tekrar tekrar veri döndürdü → **`clock_timestamp()` zorunlu, donan saatlerin hepsi yasak**, bilet ≤60 sn. (3) ONAY + sıkılaştırmalar: bilet INSERT sütunları (sahte `created_xact='2'` commit'li sayılıyordu), TOTP adımı saate bağlı (zehirli adım hesabı kalıcı kilitliyordu), yazılan damgalar `clock_timestamp()`, yazma dönüşü yalnız `void`/`uuid`, operatör rolü bilet/oturum/kimlik bilgisi/audit tablosuna dokunamaz, `op_complete_enrollment` (orkestratör kararı). (4) kapanış kontrolü ONAY; altı metin notu orkestratör kapattı. Yazar iki reçetemden ölçümle daha iyi çözüme saptı (arama terimi bilet hash'ine katlandı; var olmayan hedef `INSERT … WHERE EXISTS` ile kapandı).
- **Plan metnindeki açık:** m10-platform.md §3 (iv) `SET search_path = pg_catalog, public` **sömürülebilirdi** (çağıranın geçici tablosu definer'ın okumasını ve audit'ini ele geçirdi) — ADR `pg_catalog, pg_temp` + nitelikli adı zorunlu kıldı.
- **Ders (agent-brief'e değil, not):** tasarım denetiminde güvenlik merceği iki kez RED verdi ve ikisi de **ölçülmüş** bir sömürüydü; bir ADR'nin "her çağrı audit'lenir" gibi garantileri, transaction sınırlarını saldırganın seçtiği bir tehdit modelinde ayrıca ölçülmeden yazılmamalı.

### 2026-09-26 (27. oturum, ikinci yarı) — ✅ **M10 FAZ 0'IN KOD KISMI BİTTİ: F0-6 + OP-3 + F0-6b** (`01ec1c8` · `64f7618` · `71e7ef8` · `79d9f84` · `0f3979a`) — ✅ **DEPLOY EDİLDİ** (`478f93e`)

- **Deploy (kullanıcı kararı):** birleştirme öncesi üretim yeniden ölçüldü (goose 23, `tags` 3 satır — Rusty Bar hâlâ damgasız active, `tenants` ACL dev'le aynı, trigger yok). `main` ff → `478f93e`; main CI `success`, deploy `success`; canlı `sha-478f93e`, goose **25**, trigger 1, `tenants` `tappa_app=r`, healthz/readyz 200. Rusty Bar kartı artık "This plaque needs replacing" gösterir; Replace çalışır.

- **F0-6 `01ec1c8`** — `AssignTagToLocation` ön koşulunu ifadede taşıyor: `status='unassigned' AND encoded_at IS NOT NULL`. Mount ve replace'in yeni plaketi reddedilir; replace'te retire aynı tx'te geri alınır; eski satırın retire'ına kapı YOK (Rusty Bar onarımı Replace ile). Ret `plaque.mount_refused` (ayrı tx, en-iyi-çaba, yalnız kendi tenant'ı). **KeyState artık `encoded_at`'ten** — eski "satır varsa Encoded" varsayımı encode aracıyla yanlış olmuştu (satır adım 3'te doğar). Kart hiçbir yerde "tap reddedilir" demez (aynı NULL, işaretlenmemiş ama kişiselleşmiş çipi de kapsıyor). "Ready to mount" yalnız damgalı stoku sayar. Seed 12 demo plaketini damgalar + kayma testi. 3 yapıcı turu / 2 üçüncü göz + güvenlik ONAY. **Takip `64f7618`:** ret audit'i istek iptalinden ayrıldı (`WithoutCancel` + 5 sn, `markEncoded` emsali), test sırayı ve süre sınırını pinliyor.
- **F0-5 CI kırmızısı → `71e7ef8`:** `TestPrePush_AnInterruptedScanLeavesNothing/terminated` CI'da düştü. İki kusur ölçüldü (Docker ubuntu 24.04 bash 5.2, yük altında): geçici yarış (TERM'de ana süreç alt kabuğun temizliğinden önce çıkıyordu, 18–32/300) ve **gerçek sızıntı** (alt kabuğun kendi dizini SIGHUP'ta süreç grubu bittikten sonra da kalıyordu, 22–28/300 — satır metni TMPDIR'da). Çözüm: çalışma dizininin sahibi ana kabuk; INT/QUIT/HUP/TERM/PIPE tuzakları; miras kalan `R7D_WORKDIR` unset; boş olmayan dizin reddi. 1600 + 200 koşuda kalıntı 0. Dal CI'ı yeşil.
- **OP-3 `79d9f84` (migration 00024):** `tappa_app`'in `tenants` UPDATE'i `name`/`business_type`/`timezone`'a daraldı (`vat_number`, `structure` düştü) ve **kullanılmayan DELETE kaldırıldı** (ölçüm: kod hiç silmiyor; orkestratörün kapsam genişletmesi). Üretim ACL'i ölçüldü = dev. Migrator, brief'teki "kemer" REVOKE'unun Account kaydetmeyi 42501 ile kıracağını **ölçümle itiraz ederek** gösterdi.
- **F0-6b `0f3979a` (migration 00025):** `tags_active_requires_recorded_encode` trigger'ı `active`'e **geçişi** her rol için reddediyor (OLD ve NEW damga — tek ifadede "damgala + mount" da geçmez); tap sayaç UPDATE'i etkilenmiyor (§4.6, ölçüldü). CHECK ölçülüp elendi (NOT VALID tap'i reddediyor, DETAIL tuple'ı basıyor). **Açık, sayılı:** doğrudan `active` INSERT (DEFAULT `'active'`) → backlog **T76**. Backlog **T75**: `in_service` sayacı damgasız active plaketi sayıyor.
- 🔴 **Süreç kazası (dev DB):** migrator'ın zsh değişken bölmemesi yüzünden `up-by-one` koşmadı, `;` ile zincirlenmiş iki `migrate-down` 00023 ve 00022'nin Down'unu koştu → dev'deki `encoded_at`/`app_key_ref` değerleri silindi (şema geri kuruldu, seed yeniden damgalandı; **üretim etkilenmedi**). Ders agent-brief'e: zsh `$VAR`'ı bölmez, migration komutları zincirlenmez.
- **Kalıntı (kullanıcıda):** dev Postgres'te bir denetçinin bıraktığı `tappa_f06aud1` veritabanı (DROP bir kanca tarafından engellendi; komut: `docker exec tappa-db sh -c 'psql -U "$POSTGRES_USER" -d postgres -c "DROP DATABASE tappa_f06aud1"'`). Dev DB'de "F0-6 UI check" test tenant'ları duruyor.

### 2026-09-25 (27. oturum) — ✅ **M10 FAZ 0 İLK PARTİ: F0-5 SIR KAPISI + F0-7 + OP-2** (`e226eea` · `3844ea1` · `30284ce` · `ea43e4d`, dal `m10-faz0`, pushlu)

**Yapılan (dört commit, dal CI'ı sonrası main'e birleştirme = deploy = kullanıcı kararı):**
- `e226eea` **docs(plan)** — 🔴 **ORKESTRATÖR HATASI: main CI `b09ba1e`'den beri KIRMIZIYDI** ve 26. oturumda fark edilmedi: M10 planının WL-5 satırı henüz yazılmamış bir test adına atıf yapıyordu → `TestEveryNamedTestExists` (55 live / 54 budgeted). Deploy atlandı (yalnız belge), canlı etkilenmedi. Atıf test-adı biçiminden çıkarıldı. **Ders: main'e her push'tan sonra — belge commit'i dahil — CI conclusion'ı JSON'la kontrol et; `deploy skipped` sessizdir.**
- `3844ea1` **F0-7** — `isReEncodeRejection`: `changekey*`+911E/919D VEYA tam `writedata`+91AE → `already-encoded` (`sun.SWAuthenticationError`). `authenticate.2`+91AE (anahtar-0'ı döndürülmüş çip) **bilerek `refused`**: silikonda ölçülmedi ve boş çipte bozuk el sıkışması aynı kodu verir → önce yedek bir md.5 çipiyle ölç. Sürücü adım adlarını pinleyen test.
- `30284ce` **OP-2** — `operatorAdminIDs` testleri (kapsam %11 → %100); kart düzeltmesi (`a,,b` ayırt edici değil, `<uuid>,,<uuid>` + mesaj).
- `ea43e4d` **F0-5** — `scripts/secretscan.sh` (desenler + muafiyet tablosu tek yerde) · redline **R7d** (commit'lenebilir her dosya, `docs/`/`.claude/` dahil, `.env` hariç) · `scripts/git-hooks/pre-push` (push aralığının EKLENEN satırları + commit/tag mesajları + ham nesne + mergetag; git ayarları sabitlenmiş) · `make hooks` · CI `make audit` önceki adım kırmızıyken de koşar. **Bulgu metni hiçbir çıktıya basılmaz** (public CI log'u). Muafiyet = **satırın tüm baytlarının sha256'sı** (64 girdi). Kaçan biçimler `secretscan.sh` başlığında **33 sayılı sınır** + "bilinen gürültü"; tamlık iddiası yok. A-0'ın gerçek commit'i `a57d6e7` → 4 bulgu, `920baaa` → 0 (yalnız sayı ölçüldü). **Bu makinede kanca KURULU** (`core.hooksPath=scripts/git-hooks`); başka klon `make hooks` koşmalı. Yeni bir uzağa ya da URL ile push tüm geçmişi tarar ve A-0'ın 9 satırı yüzünden exit 1 verir — push daima uzağın ADIYLA.
- **Yan bulgu (önceden var):** GNU `mktemp -t ad` X'siz şablonu reddediyor → redline'ın "tarama patladı → exit 2" işaretçisi Ubuntu CI'da **yazıldığından beri ölüydü**; düzeltildi.

**Tur sayısı:** F0-5 **12 yapıcı turu / 11 üçüncü göz + 1 tappa-security-auditor; 7 RED** (1–6 ve 8), ONAY 7/9/11/12 + güvenlik. F0-7/OP-2 1. turda ONAY.

**Öğrenilen (agent-brief'e de yazıldı):**
1. 🔴 **Satırın İÇİNDEKİ bir belirteci affeden muafiyet altı turda altı kez aşıldı** (önek · aynı satır ayraçları · satırlar arası · tırnak paritesi · belirteç içindeki tetik · boşluğa çevirmenin açtığı sol uzatma). Sıkılaştırmak her seferinde yeni bir dilbilgisi kaçışı doğurdu; **muafiyet satırın tamamına (sha256) bağlanınca sınıf yapısal olarak kapandı** — redline R1'in "cümleye bağlı" ilkesi. 4. turda ayrıca kök neden kaldırıldı: zayıf dev değerleri (`tappa`) hiç yakalanmıyor (değer gücü), yani muafiyet gerekmiyor.
2. 🔴 **Orkestratör hatası (8. tur):** ağaçta da geçmişte de olmayan bir yanlış pozitifi (boşluksuz printf) susturmak için kod-şekli istisnası istedim → `%` taşıyan güçlü değerlerin yakalanma oranı 248 → 32'ye düştü (denetçi simülasyonu). **Yanlış pozitif önce "bilinen gürültü"ye SAYILIR; kod-şekli istisnası yanlış negatif doğurur.**
3. Yapıcı, "pinlenemiyorsa kaldır" dediğim HUP tuzağına **ölçümle itiraz etti** (Ubuntu bash 5.2 tuzaksız 12/20 koşuda TMPDIR'da satır metni) — kabul edildi, tuzak kaldı (#31).
4. Güvenlik denetçisi, F0-5 kartının saymadığı §4.7 merkezini buldu: KEK/plaket anahtarı A-0 biçiminde sessizdi → 11. turda kapatıldı (anahtar bağlamı + `kek`/`hmac_key` atama anahtarları + rotasyon sonekleri + camelCase).

**Sürüyor:** F0-6 (mount kapısı + KeyState'in `encoded_at`'ten türemesi — bugün "Encoded" sözcüğü yalnız satırın varlığından türüyor, Rusty Bar'ın yarım plaketi panelde "Encoded" görünüyor) yapıcıda. Sonra OP-3 (migration 00024, tappa-db-migrator). **Kullanıcıya borçlu (değişmedi):** operatör panel parolası ROTATE (A-0) · Rusty Bar plaketi yeni çiple Replace (A-1) · `tappa_app`/`tappa_owner` DB parolaları ROTATE · SES dış adımları (plan §4).

### 2026-09-19 (23. oturum, ikinci yarı) — **DÖRT PİLOT-ÖNCESİ İŞ PLANLANDI · İŞ 1 (İMPRİNT + OPERATÖR SEAM) DONE** (`b0eb4d7`)

**Kullanıcı sordu:** "bugün adb/DB/Playwright ile yaptıklarımın hepsini bir operasyon admini ARAYÜZDEN yapabilir mi?" **Cevap ölçüldü:** günlük operasyonun çoğu panelde (çip mount · lokasyon GPS/IP · çalışan+davet · raporlar — bugün hepsi PANELDEN Playwright ile yapıldı). Panel dışı ama **tasarım gereği**: encode (Android uygulaması) · aktivasyon+tap (personel telefonu). **Gerçek eksikler: T73 · Q02 · M9-08.**

**Dört iş sırayla planlandı** (bir keşif ajanı çıkardı, sıra doğrulandı): **1) İmprint → 2) T73 → 3) Q02 → 4) M9-08.** Kullanıcı kararları alındı.

🔑 **KRİTİK KEŞİF — "operatör" tasarımı:** ürün **bilinçle AYRI OPERATÖR ROLÜ YAPMADI** (`legaladmin.go` yorumu: *"NO SUPER-ADMIN, no third role"*). `admin_users.role` kapalı sözlük (`owner`,`manager`, ikisi tenant-scoped, policy motoru `actor:role` okur). Operatör = mevcut owner/manager + **`config.OperatorAdminIDs` allow-list** (`admin_users.id`, env `TAPPA_OPERATOR_ADMIN_IDS`) + **operatör-only ekranlar** (`PanelSection.OperatorOnly`). Legal admin (M7-06) ilk örnek. **M9-08 = bu modeli genişletmek** (ayrı `/operator/*` ağacı + billing/VAT/plaket ekranları), yeni rol DEĞİL.

**✅ İŞ 1 DONE.** Codevera Labs künye bilgisi `legal_documents`'e (append-only, tenant_id YOK, ADR 0016) **`tappa_owner` ile direkt INSERT** (kullanıcı "DB'ye direkt" dedi; `published_by` NULL). ⚠️ **Snapshot tuzağı:** `/legal/imprint` in-memory `Published()` snapshot'tan okuyor (startup'ta yüklenir) — DB direct INSERT snapshot'ı yenilemedi. **Çözüm İş 4 ile birleşti:** `TAPPA_OPERATOR_ADMIN_IDS`'e **iki Ozcan id'si** eklendi (`9bdf19a4…`, `07f4f6bc…` — aynı e-posta, iki tenant, kullanıcı "dokunma, ikisini operatör yap" dedi) → redeploy → snapshot reload → imprint canlı **VE** operatör mekanizması ilk kez aktif. Doğrulandı: `/legal/imprint` Codevera gösteriyor, Ozcan `/admin/legal`'e erişiyor.

**🔴 KALAN İKİ İŞ — sıradaki İŞ 3 (Q02):**
· **✅ İŞ 2 — T73 DONE** (`38ddbaf`, dal `t73-admin-change-password`, main'e merge EDİLMEDİ; üçüncü göz + tappa-security-auditor ONAY). *(Aşağısı uygulanan planın tarihsel kaydı.)* mekanizma HAZIRDI — `internal/adminauth/password.go` (`Hash` bcrypt cost 12, `Compare`), `signup.MinPasswordRunes=12`, `adminreset.go` `checkNewPassword`, `RevokeAdminSessionsForAdmin` (`admins.sql:175`). **EKSİK:** (a) `admins.sql`'e 2 sorgu — id+tenant ile `password_hash` OKU (mevcut parolayı doğrula) ve GÜNCELLE (`GetAdminByID` password_hash seçmiyor; migration GEREKMEZ) · (b) yeni handler `POST /admin/account/password` (`mountWriting`, ProtectWriting; desen: `accountactions.go:106` `accountSave`) · (c) Account formu (`account.templ:190`) · (d) testler (handler + DB/RLS + `-race`). **Karar K3:** parola değişince **diğer oturumlar iptal, mevcut hariç** (session id hariç `RevokeAdminSessionsForAdmin`). `tappa-db-migrator` (sorgular) + `tappa-brand` (form) + `tappa-security-auditor` (§4.7 auth).
· **İŞ 3 — Q02 (e-posta, orta, SAĞLAYICI KARARI BEKLİYOR, ADR gerekli):** seam HAZIR — `internal/invite/channel.go` `Channel.DeliverInvite` (bugün `ManagerVisibleChannel` = link panelde; ADR 0005 Y-D, Q02 çözülünce çalışan kanalına taşınmalı, "zorunlu") · `adminreset.go` `ResetChannel.DeliverReset` (`mail` nil olabilir) · `config.ResetDelivery` (env `TAPPA_RESET_DELIVERY`, tek değer `none`) · `main.go:592` switch (`case` eklenecek). **Entegrasyon = tek Channel implementasyonu + config case + davet kanalını değiştir.** **§1: sağlayıcı SDK yerine `net/smtp` (0 dep) ya da `net/http` (SES SigV4) — yeni bağımlılık ÖNCE SOR.** 🔴 **KULLANICI KARARI: sağlayıcı** — öneri **AWS SES (eu-central-1, AB residency + DPA net)**. Rapor gönderimi seam'i YOK (ileride ayrı Channel).
· **İŞ 4 — M9-08 (operatör paneli, BÜYÜK, §4.5 tenant sınırını bilerek aşar, ADR + ağır güvenlik):** karar **B — mevcut allow-list modelini genişlet** (ayrı rol değil). Kart (`m9-sonrasi.md:120`): `/operator/*` ayrı rota ağacı (müşteri oturumu giremez), her eylem audit_log + hangi tenant adına, boş liste=kimse (testle sabit), handler 403 (404 değil). Kalan ekranlar: legal sürüm geçmişi/geri-alma · **tenant listesi/arama/askıya alma** (yok) · billing operatör tarafı (`00016` `tappa_app`'e kapalı) · VAT yeniden doğrulama · plaket envanteri. **PİLOT SONRASI (M9).**

🔴 **KULLANICIYA BORÇLU:** **canlı `tappa_app` + `tappa_owner` DB şifreleri bu oturumun sohbetinde açığa çıktı → ROTATE.** İki owner hesabı (çift signup, dokunulmadı). Üretim tenant'ında test verisi kalıcı (Ozcan Test + tap kayıtları, §4.3). **md. 5** (anahtar 0 şeması) ✅ **KOD OLARAK KAPANDI** (25. oturum, `95cf32d`, ADR 0018 + step 8) — kalan: **gerçek çipte step 8'i koşmak** (geri-dönülemez, kullanıcı kararı). Bu yapılmadan çip hâlâ fabrika anahtarında, duvara çıkamaz.

### 2026-09-24 (26. oturum) — 🔴 **OLAY A-0 + M10 PLANI** (white-label · SES · süper admin)

**Kullanıcı isteği:** "white-label (restoran kendi logosu/renkleri), AWS SES ile e-posta, süper admin var mı — her restoran admini sistem verisini güncelleyebiliyor; detaylı plan yap, bu tarz problemler önce çözülmeli." Üç mimar ajanı (Plan, salt-okuma) paralel koştu; orkestratör birleştirdi → **`docs/plan/m10-platform.md`** (Faz 0 + üç akış, ~40 görev, kabul kriterleri, kararlar).

🔴 **OLAY A-0 (süper admin ajanı buldu):** operatör hesabının (allow-list'teki tek id = KF Ltd. owner'ı) canlı panel parolası, BENİM yazdığım "rotate edilmeli" borç notlarının içinde state.md'de düz metin (4 satır, 3 commit, 2026-09-19'dan beri pushlanmış) — ve **depo `atknatk/tappa` PUBLIC** (ölçüldü). Kök neden: sırrın DEĞERİ bir kayıt dosyasına yazıldı. **Triage (salt-okuma):** hesabın 9 oturumu + 9 `admin.login.succeeded` satırının hepsi bilinen etkinlik (19 Eylül kurulum otomasyonu + kullanıcının cihazları); 20 Eylül 08:44'teki tek istek MEVCUT çerezle `GET /admin`; başarısız giriş 0; 19 Eylül'den beri tek yasal yayın bizim künye → **yetkisiz kullanım kanıtı yok.** DB parolaları ve eski panel parolası repoda/geçmişte YOK (ölçüldü). Değer HEAD'den kaldırıldı (`920baaa`, yerel). Ders hafızaya ("sır asla repoya yazılmaz") ve M10 F0-5'e (redline sır kapısı). **Kullanıcıya:** F0-1 parolayı panelden değiştir (yeni parola sohbete/repoya yazılmaz) · F0-3 depo görünürlüğü · F0-4 DB parolaları.

**Endişenin denetimi:** 24 panel POST rotası tek tek → **kodda yetki açığı YOK** (yasal metinler allow-list dışına 403, `sys:*` kodda, plan/fiyat/VAT sütunları tappa_app'e kapalı, RLS). Kullanıcının gördüğü "company details" = yasal metin sekmesinin açıklaması, yalnız kendi (operatör) hesabında çiziliyor. Asıl sorun YAPISAL: platform gücü bir restoran hesabına bağlı (MFA yok, sunucu tarafı oturum süresi yok, audit restoranın tenant'ına) → önceki "B — allow-list genişlet" kararının yerine **(c) hibrit** önerildi (M9-08'in "müşteri oturumu /operator'a giremez" kriteri (a) ile yapısal olarak karşılanamaz).
...
### 2026-09-20 (25. oturum) — ✅ **md.5 DONE: ANAHTAR-0 (AppMasterKey) ROTASYONU / STEP 8 SEVK EDİLDİ** (`95cf32d`) — encode hattının kripto çekirdeği tamamlandı

**Kullanıcı sordu:** "mobilde istediğimiz şubeye istediğimiz gibi kart yazabiliyor muyuz? kartları admin'den şubeye atayabiliyor muyuz?" Cevap ölçüldü: **mount panelden TAM** (`plaqueactions.go` `mountPlaque`/`Unmount`/`replacePlaque` — çip↔şube), **encode batch destekli** (20/pencere) ama **şube-agnostik** (şube mount ile). 🔴 **Kritik engel: md.5** — key-0 fabrika default'ta, çip "duvara çıkamaz". Kullanıcı iki soru cevapladı: (1) sıradaki iş = **md.5** (Q02 yerine, kart odaklı), (2) anahtar-0 şeması = **ayrı sütun** (KDF ve yaz-at elendi).

**Karar (ADR 0018):** anahtar-0 plaket-başına rastgele AES-128, key-1'den bağımsız, KEK-sarmalı, AYRI sütun `tags.app_key_ref`. Elenen iki şık ölçümle: **KDF** (SDM ve master'ı tek sırra kenetler, ADR 0017 §5.0 karar-1 ihlali) · **yaz-at** (§5.3 kurtarma ölür, çip brick+teşhis edilemez, §4.6 ihlali).

**A/B faz bölmesi (kripto işi, en riskli — step 8 encode'un tek geri-dönülemez komutu):**
· **A — tappa-db-migrator:** migration 00023 (`app_key_ref bytea` nullable, yaz-bir-kez trigger `IS DISTINCT FROM`, 44-bayt CHECK, GRANT eklenmez — INSERT tablo-düzeyinden gelir, UPDATE/SELECT verilmez = aes_key_ref emsali/§4.7 en az yetki), `InsertUnassigned`'a `app_key_ref` parametresi. **Down v22 ile bit-birebir** (sha256 eşleşti). RLS + yaz-bir-kez testleri.
· **B — builder (opus, tappa-sun):** `session.go` step 3'te ikinci bağımsız `mintPlaqueKey` + app_key_ref'i aes_key_ref ile AYNI INSERT'te yazar (§5.2 DB-önce); `driver.go` step 8 `changekey.appmaster` (en son, ChangeFileSettings'ten SONRA çünkü oturumu bitirir), `acceptSessionEndedAck` (MAC'siz 9100, unwrap yok); fake chip key-0-sonrası oturum ölümü. Handler encode-budget'ı türev olduğu için otomatik kaydı (RequestsPerRound 11→12 → limit 240, headroom 59).

**İki bağımsız denetim, ikisi kendi komutuyla üretti, ONAY:** üçüncü göz (Down bit-birebir, iki anahtar farklı, sıra, oturum ölümü) + tappa-security-auditor (§4.7 düz anahtar sınırı, key izolasyonu, yaz-bir-kez orphan guard, cross-tenant RLS, KAT case-2). **İki bloklamayan bulgu commit'ten önce kapatıldı:** F1 (app-zarfı uzunluk kapısı negatif birim testi — **mutasyonla kanıtlandı**: kapı devre dışı→kırmızı, geri→yeşil), F2 (dangling-citation gerekçe notu). Orkestratör doğrulaması: build/vet ok, go.mod/sqlc diff boş, redline exit 0, kapsam 22 dosya.

**Ne kaldı.** 🔴 **GERÇEK ÇİPTE step 8'i koşmak** — geri-dönülemez (çip fabrika anahtarını kaybeder), donanım kullanıcıda, kullanıcı kararı. Bu yapılınca çip GERÇEKTEN duvara çıkabilir. SIRADAKİ **İŞ 3 — Q02** (e-posta). ⚠️ md.5 dalı (`md5-key0-rotation`) T73'ün ucunda stacked; **main'e merge edilmedi** (merge sırası: T73 → md.5, kullanıcı deploy kararı). ⚠️ pre-existing sapma: `cmd/rotatekek` GOPACKAGESDRIVER (T72 sınıfı, backlog). 🔴 canlı DB şifreleri + Ozcan parolası (`[parola repodan kaldırıldı]`) → ROTATE.

**Deploy (25. oturumun ikinci yarısı) — T73 + md.5 CANLIYA.** Kullanıcı "kart yazabiliyor muyuz" sorusundan sonra "deploy et" dedi. Güvenli deploy akışı (CLAUDE.md §10: önce dal CI, sonra main): md5 dalı (T73'ün ucunda stacked, ikisini taşır) push → dal CI. 🔴 **CI İLK TURDA KIRMIZI: `storekeyshape_test.go` gofmt-uyumsuz commit'lenmişti** (ajan map'e girdi ekledi, gofmt hizalamasını çalıştırmadı; ben orkestratör doğrulamasında `make check` ATLADIM — T72 kırmızısı varsayıp build/vet/redline ile yetindim → gofmt farkını kaçırdım). Düzeltildi (`402a6fe`, `gofmt -w`, davranışsız), dal CI yeşil → main'e ff-merge + push. 🔴 **main CI İKİNCİ KEZ KIRMIZI: `TestGPS_IsUnloggable` FLAKY** — dal CI'da (aynı commit) geçmişti; kök neden `gpsLeaked` yasak listesindeki kısa eksen prefix'i `"35.9"` log TIMESTAMP'iyle (`:35.9`45Z) çakışıyor, redaction'ın kendisi çalışıyor (`gps="[REDACTED gps]"`). rerun geçti → deploy `35467987824` success, **T73+md.5 canlı** (healthz/readyz 200, imprint korundu).

**Flaky kalıcı düzeltildi (`29842dd`).** gpsredaction_test.go (T65) + geo/redaction_test.go (aynı latent kusuru taşıyan twin): test slog handler'ına `noTime` ReplaceAttr eklendi (timestamp düşer), yasak liste + REDACTED assertion DEĞİŞMEDİ, üretim redaction koduna dokunulmadı. Builder mutasyonla kanıtladı (redaction kırılınca test yakalıyor, -count=2000 -race yeşil), **tappa-security-auditor ONAY** (§4.7 kontrol zayıflamadı). main'e merge + deploy `35469750408` success → CI artık kararlı.

**Çift-tenant temizliği + tek temiz hesap + parola min 8 (25. oturumun üçüncü yarısı).** Kullanıcı "niye 2 tane aynı user var" sordu. Teşhis (kubectl `hetzner-k8s-1` context, `tappa-postgres-0`, pod'un KENDİ `$POSTGRES_PASSWORD`'üyle — şifre çıktıya alınmadı, salt-okuma): aynı e-posta (`ozcan@kebabfactory.com.mt`) İKİ tenant'ta owner — "Kebab Factory" (sahte VAT MT12345678, 29 Ağu, çip+10 tap pilot verisi) + "Kebab Factory Ltd." (gerçek VAT MT22339606, 15 Eyl, 9 boş lokasyon). İki kez signup (tenant-scoped e-posta uniqueness). Kullanıcı "hepsini temizle 1 temiz aç" dedi. 🔴 **İD TUZAĞI YAKALANDI:** ilk envanterde `9bdf19a4`/`07f4f6bc`'yi tenant_id sandım — bunlar **admin_id**; doğru tenant_id'ler `fd0b7c61`/`5917c5db`. Yanlış id ile silme 0 satır etkilerdi (envanter bunu gösterdi, felaket önlendi). **Silme:** doğru tenant_id'lerle, 16 FK-RESTRICT child tablo + tenants, superuser tek transaction (`session_replication_role=replica` + `--single-transaction ON_ERROR_STOP=1`), silme öncesi/sonrası sayı doğrulaması (10 tap+1 çip+2 admin+21 session+56 audit… = envanter, sonrası 0). ⚠️ §4.3 (transactions immutable) test verisi + tenant kapanışı olduğu için, kullanıcı açık onayıyla aşıldı. **Signup:** Playwright, Kebab Factory Ltd. + MT22339606 + `[parola repodan kaldırıldı]` (14 char, o an min 12), bot-defence (8sn min-fill + `internal_ref` boş, radio'lar `force:true`) → **VAT VIES-onaylı** (tenant `4931dd67`, admin `f3c91606`). **MinPasswordRunes 12→8** (`1739c65`, ADR 0019, üçüncü göz ONAY — signup/reset/T73 üç akış tutarlı) + **allow-list `f3c91606`** (`fca48c2`) → main `fca48c2` deploy `35474254869` success. **T73 ile parola `[parola repodan kaldırıldı]`'e düşürüldü** (canlı, "At least 8" formu, taze context'te doğrulandı). 🔴 **Canlı tek hesap:** `ozcan@kebabfactory.com.mt` / `[parola repodan kaldırıldı]` · tenant Kebab Factory Ltd. (temiz, çip/tap yok — pilot buradan başlar). Borçlar: DB şifreleri hâlâ ROTATE · gerçek çipte step 8 (md.5) · başka bir tenant DB'de var (dokunulmadı).

**Encode hata-loglama + UI sebep + md.5 Android borcu (25. oturumun dördüncü yarısı, canlı `aefa67d`).** Kullanıcı gerçek çipte encode denedi, "4. adımda hata, DNA'ya write edemedim" dedi ("sistemde logla, arayüzde güzel sebep yaz"). Teşhis (kullanıcı doğruladı: DAHA ÖNCE encode edilen çip): tekrar-encode'da `authenticate` (fabrika key 0) geçer ama `changekey.sdmfileread` (§5.1 step 6) driver fabrika key 1 varsaydığı için — key 1 artık bizim anahtarımız — çip **911E (INTEGRITY_ERROR)** döndürür. **Bir çip iki kez encode edilemez; ADR 0017 §5.3 kurtarma akışı SHIP EDİLMEMİŞ** → o çip "yakıldı", yeni boş çip gerekir. 🔴 **Loglama zaafı ölçüldü:** adım-hata log satırı `step=""` (advance boş Progress dönüyor), tenant/uid/session yok, status word `err` string'ine gömülü. **UI zaafı:** sunucu her çip hatasını jenerik `refused`'a düşürüyordu (status word'ü atıp). **Düzeltme (`966df96`, 2 denetim ONAY):** `sun.StatusError{Got,Want}` + `encode.StepError{Step,UIDHex,Err}` typed error (status word kayıpsız, Error() metni aynı → KAT etkilenmez); yapılandırılmış Warn log (step DOLU + status_word + want + uid + tenant_id + session_id + actor — §4.7 komut/anahtar/CMAC ASLA, sızıntı testiyle kanıtlı); yeni `already-encoded` fault (911E/919D + changekey → "bu plaket zaten yazılmış, yeni kullan"), diğer → refused; Android Wording spesifik mesaj + retryIsSafe=false. **md.5 Android borcu (`aefa67d`):** step 8 (changekey.appmaster) driver'a eklendiğinde Android 10'da kalmıştı — FakePanel.STEPS 11 (driver ile birebir), EXPECTED_EXCHANGES 10→11, ContractPinTest yeşil (theElevenStepNames), 33 Android testi 0 fail. Deploy `aefa67d` canlı. 🔴 **Backlog:** ADR 0017 §5.3 kurtarma akışı (yakılmış/yarım çip kurtarma) hâlâ ship edilmedi — bir çip tek encode şansı.

**Re-encode / discard keşfi + yakılmış çip elle temizlendi (25. oturumun beşinci yarısı, kod değişikliği YOK).** Kullanıcı canlıda `already-encoded` mesajını gördü (deploy çalıştı) ve "bir çibi silip tekrar yazma özelliği ekleyelim, ama tenant-scoped (başka müşteriye dokunmasın)" dedi. Keşif + iki bloke:
· 🔴 **Aynı-çip re-encode (yeni anahtarla) ADR 0017 §5.4 KIRMIZI ÇİZGİ** — *"kurtarma asla 'yeni anahtarla baştan yaz' olamaz"* (+ ADR 0003 md.5). Kullanıcının ilk isteği tam bu; reddedildi (sızmış-anahtar riski, iki-kez-encode karmaşası). Kullanıcı **retire+replace** seçti.
· 🔴 **retire+replace de unassigned/yakılmış çibi KAPSAMIYOR** — `tappa-db-migrator` çalışma-anında bloke etti (23514 ölçüldü): `tags_retired_keeps_its_location` CHECK (00013) bir `retired` çipin location'ı DOLU olmasını şart koşuyor; yakılmış çip `unassigned`+location'suz → retire edilemez. ADR §5.2 bunu bilinçle böyle bırakmış (*"yarım tur satırı unassigned kalır, hizmette görünmez, zararsız, sessiz temizlik YOK"*). Ajan doğru bloke etti (migration+ADR yasağı + §4.6, otonomi bunu geçmez).
· **Tenant güvenliği zaten sağlam** (RLS+privilege+tenant filtresi; uid global PK olsa da başka tenant görünmez/yazılamaz/silinemez) — kullanıcının kısıtı yapısal olarak var.
· **Karar (kullanıcı):** 'discarded' statü özelliği (migration+ADR) ERTELENDİ; yakılmış çibi **elle temizle** seçildi. Yakılmış çip orphan'ı (uid `0492A2BA902390`, Kebab Factory Ltd., unassigned, encoded_at NULL, 0 txn) `tappa_owner` superuser ile dar WHERE'le DELETE edildi (encode-başarısız orphan, §4.6 endişesi yok — hiç hizmet görmedi). Kebab Factory Ltd. tertemiz (0 tag). Kullanıcı YENİ boş çiple encode edecek (yakılmış fiziksel çip çöpe).
· **Backlog:** 'discarded' statü (kullanıcı ürün özelliği isterse — ADR §5.2 genişletme) · ADR §5.3 kurtarma akışı (yarım çip, aynı anahtarla). SIRADAKİ planlı: İş 3 — Q02.

### 2026-09-20 (24. oturum) — ✅ **T73 DONE: YÖNETİCİ PANELDEN KENDİ PAROLASINI DEĞİŞTİREBİLİYOR** (`38ddbaf`) — İş 2/4 bitti

**Compact sonrası devam.** Dört-iş sırasının İş 2'si (T73). Zemin doğrulandı: HEAD `598fbdb`, db healthy, go build temiz. Dal `t73-admin-change-password` açıldı (CLAUDE.md §10 — main'e merge = deploy kararı, YAPILMADI).

**Keşif (bir Explore ajanı) T73 yüzeyini haritaladı** ve kritik gerilimi ortaya koydu: tek hash-okuma yolu `resolve_admin_by_email` (pre-auth, çok-tenant SET) change-password'a uygun değil; `passwordresets.sql:156-159` zaten T73'ü öngörüp *"kendi statement'ı, kendi yetkisi — bunu yeniden kullanma"* diyor; `RevokeAdminSessionsForAdmin` "except session" almıyor. `admins.sql` header'ı *"password_hash hiçbir sorguda seçilmez"* grep-invariant'ı taşıyor — change-password mevcut parolayı doğrulamak için hash okumak zorunda, yani invariant genişlemeli.

**Orkestratör kararları (otonomi, önerili — uygulandı):** K-A change-password kapısı = herhangi bir canlı admin (owner+manager), account-detayının owner-only kapısı DEĞİL (parola kişisel) · K-B mevcut parola istenir (bcrypt Compare, oturum-çalma savunması) · K-C yeni≠mevcut · K-D iki audit olayı (changed/refused) · K-E iş mantığı `adminauth.Manager.ChangeOwnPassword` (pair-tx), handler ince (§3) · K3 (önceden mutabık) mevcut oturum hariç diğerleri iptal.

**Üç ajanlı tur:** (1) `tappa-db-migrator` → 3 sqlc sorgu (`GetAdminPasswordHashByID`, `SetOwnAdminPassword` status='active' guard, `RevokeOtherAdminSessionsForAdmin` except-session), **migration YOK** (GRANT'lar 00006/00011/00017'de mevcut — ölçüldü), header invariant grep-dürüst yeniden yazıldı (hash TAM 2 yerde SELECT: login resolver + bu id+tenant okuma; `admins.sql.go`'da PasswordHash taşıyan tek Row struct `GetAdminPasswordHashByIDRow`), RLS izolasyon + pozitif testler. (2) builder (opus) → `changepassword.go` (6 adım tek WithTenant tx, 3 sentinel), `accountpassword.go` (kapı `id.Live()`, her err için davranış, route mountWriting/ProtectWriting miras), account formu (her canlı admin'e çizilir), testler. ⚠️ builder R7'yi kendi buldu: 7 log/error string'i "password"→"credential" (redline "password" kelimesini log/fmt içinde tarıyor, muafiyet felsefesi bu kelimeyi affetmiyor); audit action/tip/render string'leri "password" kaldı (taranmıyor).

**İki bağımsız denetim, ikisi de kendi komutuyla yeniden üretti, ONAY:** üçüncü göz (genel doğruluk — K-A manager non-owner uçtan uca, K3 except-session server-side, make gen determinizmi) + `tappa-security-auditor` (§4 — §4.7 hash sınırı grep-dürüst, §4.5 tenant belt+RLS `assertAppRole` altında, §4.6 audit dürüstlüğü, CSRF/Origin miras). Sıfır §4 ihlali, sıfır sır sızıntısı (log/audit/render/return). Orkestratör doğrulaması: go build exit 0, vet temiz, redline exit 0, kapsam 15 dosya, mühür alındı.

**Ne kaldı.** SIRADAKİ **İŞ 3 — Q02** (e-posta, seam hazır, 🔴 **sağlayıcı kullanıcı kararı bekliyor** — öneri AWS SES eu-central-1, ADR gerekli, §1 yeni bağımlılık ÖNCE SOR). Sonra İŞ 4 — M9-08 (operatör paneli, pilot sonrası). ⚠️ T73 dalda, **main'e merge edilmedi** — kullanıcı deploy kararı verince. 🔴 **Kullanıcıya borçlu:** canlı DB şifreleri + Ozcan panel parolası (`[parola repodan kaldırıldı]`) sohbette açık → ROTATE · md. 5 (anahtar 0) 25. oturumda KOD olarak kapandı (kalan: gerçek çipte step 8) · üretim tenant'ında test verisi kalıcı · iki owner hesabı.

### 2026-09-19 (23. oturum) — 🎉 **İLK ÇİP ENCODE EDİLDİ VE UÇTAN UCA TAP DOĞRULANDI** — md. 16 KAPANDI · **iki silikon düzeltmesi** (`fd1b667` plain WriteData, `4dd6959` CmdCtr)

**Kullanıcı gerçek bir NTAG 424 DNA çibini Galaxy A15 + Android rölesiyle encode etti, sonra personel gibi tap yaptı.** Bu, on bir turluk sunucu tarafı + Android rölesi + tap karar motorunun **tamamının gerçek donanımda** nihai doğrulaması.

**Encode — üç adımda, ikisi silikon düzeltmesi.**
· **1. tur:** sekiz adım geçti (`select · getversion×3 · authenticate×2 · getcarduid`), `writedata` **917E LENGTH_ERROR**. 🔴 **EV2 kripto çekirdeği (AES-CMAC oturum anahtarları) GERÇEK SİLİKONDA ÇALIŞTI** — on bir turluk sunucu tarafının hiçbiri silikon görmemişti.
· **`fd1b667` — plain WriteData:** kök neden datasheet+KAT ile kanıtlandı — fabrika NDEF dosyası (File 02) `Eh` free-access'te ve datasheet **üç yerde** CommMode.**Plain** zorunlu kılıyor; sürücü **Full** (şifreli+MAC) gönderiyordu → uzunluk uyuşmazlığı. `katT17`'nin Full olması fabrika-dışı config (§5.4). Yeni `WriteDataPlainCommand`. **1 yapıcı / 2 denetim (üçüncü göz + güvenlik), ikisi ONAY.**
· **`4dd6959` — CmdCtr:** 2. gerçek turda `writedata` geçti, `changekey` **911E INTEGRITY_ERROR**. 🔴 **Gerçek NXP silikonu plain WriteData'yı CmdCtr'a SAYIYOR** — `fd1b667` §9.1.2'nin harfine göre saymadığını varsaymıştı, silikon aksini söyledi. `cmdWriteNDEF`'e `s.useCtr()` geri eklendi (dönüş atılır, sadece sayaç ilerler). **1 yapıcı / 1 denetim, ONAY.** *(Her iki düzeltme de "anahtar değişmeden yakalandı → çip kurtarılabilir" sınıfında — tam da fd1b667'in call-site'ta öngördüğü.)*
· **3. tur:** `begin` + **on step, hepsi 200, hata yok** → `encoded_at` damgalandı. 🎉 **İlk gerçek encode edilmiş çip.**

**Uçtan uca pilot senaryosu — panel (Playwright ile) + fiziksel tap.** Kebab Factory / St Julians'ta: çip **duvara mount + active** · lokasyona **GPS + IP** · personel **"Ozcan Test"** + davet linki (Q02 çözülü: link panelde "shown once") · **aktivasyon** (adb ile Chrome'da, oturum kuruldu) · **tap**. 🔴 **Bir tuzak: NFC NDEF URL'yi VARSAYILAN tarayıcı açıyor** — aktivasyon Chrome'daydı ama tap Samsung Browser'da açılıp oturumsuz kaldı (`/t → 303 /activate`); Chrome varsayılan yapılınca çözüldü. Çerezler **SameSite=Lax**, NFC tap'te gidiyor.

🎉 **TAP MOTORU HER §5 KARARINI GERÇEK BİR TAP'TE VERDİ:** `sun_valid=true` (çip geçerli SUN, ctr 6→10) · 1. tap **practice** · 2. tap **debounce ignored** (60sn) · 3-4. tap **check-in→check-out çifti** ama `base:gps-conflict-review` (IP "St Julians" derken telefonun GPS'i Slovenya → **sistem çelişkiyi yakaladı**, akıllı güven kararı) · venue GPS kaldırılınca 5. tap **`verdict=ok`, trust=70, `base:ip-or-gps-ok`** — temiz check-in.

**Ne kaldı.** **B3b'nin fiziksel yarısı DOĞRULANDI** — encode + tap gerçek donanımda çalışıyor. Kalan: **md. 5** (anahtar 0'ın şeması — çip duvara çıkamaz, fabrika key 0; §5.1 adım 8 sevk edilmedi) **kullanıcı kararı** · **backlog T74** (step-5 response-MAC / NDEF geri-okuma) · **T73** (yönetici kendi parolasını değiştiremiyor) · **T72** (yerel Go 1.27 sapması). ⚠️ **Üretim tenant'ında (Kebab Factory) test verisi kalıcı** (Ozcan Test + 5 tap, `transactions` §4.3 değişmez) — gerçek pilot temiz tenant ister. 🔴 **Kullanıcıya: canlı `tappa_app` + `tappa_owner` DB şifreleri bu oturumun sohbetinde açığa çıktı — ROTATE edilmeli.**

### 2026-09-18 (22. oturum) — **M8-05 FAZ B3a DONE** (`4fc5e3f`) · Android APDU rölesi · **3 yapıcı turu / 2 bağımsız denetim, 1 RED** · 🔴 **KOD BİTTİ, ÇİP SIRADA**

*(Günlükte 18–21. oturumların girdisi yok; başlık 21'de duruyordu, bu 22.)*

**Kullanıcı *"yetkin var, bunları sen yap"* dedi.** Araç zinciri kuruldu (sistem, repoya girmedi): `adb` (`android-platform-tools`) · Android SDK zaten vardı (`platforms;android-36`, `build-tools;36.0.0`) · **Gradle 9.7.1** · JDK 21 (⚠️ `JAVA_HOME`'suz `gradle` bu makinede **openjdk@25** seçiyor — Makefile 21'e pinli). **Galaxy A15 (SM-A155F, Android 16) USB ile bağlandı ve okundu:** NFC açık, `mIsReaderOptionEnabled=true`, NXP firmware `N12.21.25.12`, Secure NFC kapalı.

**Ne yapıldı.** `android/` — Kotlin, Go modülünün dışında, `make android`/`make android-test`. Uygulama **kripto bilmez, karar vermez**: sunucunun `command`'ını `IsoDep.transceive`'e verir, `rapdu`'yu `POST …/step`'e gönderir, on alışveriş. Giriş **WebView + `CookieManager`** — panelin CSRF'i, çoklu-tenant *choose* adımı, flood kapısı hepsi panelin işi olarak kalır; parola uygulamada **tutulmaz**. **Tek runtime bağımlılık `kotlin-stdlib`** (`androidx` bile kullanılmadı). **33 JVM testi** sahte sunucu + sahte çiple tam turu, `done` kuralını, yedi fault'u, `TagLost`→tek abort'u, `Origin`'i sürüyor; **17 mutasyon, 0 hayatta kalan**. **Cihaza kuruldu, açıldı, FATAL 0**, `Origin` telefonda kanıtlandı (`originAccepted=true`).

🔴 **ÜÇÜNCÜ GÖZÜN BULDUĞU ŞEY, BU AİLENİN BİLDİK KUSURUYDU — "kapı var ama koşmuyor".** `ContractPinTest` Go kaynağını okuyup sözleşmeyi çiviliyor, **ama Go dosyaları Gradle task girdisi değildi** → sıcak ağaçta `UP-TO-DATE`, **altı sapmanın altısı yeşil geçti** (rota · fault · adım adı · 11. adım · form alanı · çerez adı). README *"drift → red"* garantisi veriyordu; garanti yalnız **temiz build'de** doğruydu. Çare bir satır (`inputs.files`) + **kendi kendini sınayan test** (okunan her yol build dosyasında geçmeli). Aynı sınıf B2c-2b'de *"sayım var ama düzyazıya bağlı değil"*, T64'te *"kapı var ama yürüyüş `else if`'e kör"* olarak çıkmıştı.

🔴 **GÜVENLİK MERCEĞİ YİNE SAYIMI DÜZELTTİ.** README *"holds no secret material"* diyordu; **ADR 0017'nin kendi §3'ü** şifreli `ChangeKey` gövdesinin **döküm altında anahtara eşdeğer** olduğunu yazıyor. Denetçi ölçtü: debug APK'da `am dumpheap` **root'suz** çalışıyor (39 MB); canlı bir turda döküm **panel oturumu + canlı handle + o plaketin `K_SDMFileRead`'i**ni verir. Grep **sözlük** yokluğunu kanıtlıyordu, **bayt** yokluğunu değil. Çare kod değil, **sayım**: *"telefon anahtar tutmaz"* doğru, *"sır tutmaz"* yanlış — README artık farkı yazıyor. Denetçinin kendi cümlesi: *"ADR §2.2 altı yolu denemiş ve saymış; dördüncü kapı önermiyorum."* Ayrıca: autofill'in yönetici parolasını **Samsung Pass'e** kaydetmesini engelleyen bayrak · release'te `http`'yi **bizim** reddetmemiz · `https://TapTime.mt` yazan operatörde **girişin tamamlanamaması** (harfe duyarlı karşılaştırma; test `.lowercase()` ile geçirdiği için görmüyordu).

⚠️ **Bu makinede Go 1.27.1**, `go.mod` **1.26.2** → `make check` **yerelde** iki yerden kırmızı (staticcheck sürümü · `TestRotateScript_AccountsForEveryGoToolchainVariable`, Go 1.27'nin `GOPACKAGESDRIVER`'ı), **CI yeşil**. Backlog **T72**.

**Ne kaldı — B3b, ve donanım artık kullanıcının elinde.** Telefon bağlı, APK kurulu, sunucu canlı. **İlk gerçek tur md. 16'yı kapatır** ve üç silikon sorusunu cevaplar (md. 7 CommMode · md. 8 `ChangeKey` sonrası oturum · Tablo 65). ⚠️ **İlk tur ÜRETİME yazar** — silinemez `tags` satırı, üretim KEK'iyle sarılı anahtar; o çip **laboratuvar plaketi** olur, **duvara çıkamaz** (md. 5 açık). **Release imzası yok** (debug APK operatör artefaktı değil). **Sunucu boşluğu, devir md. 29:** `p.Done` kolu fault kelimesi taşımıyor — *"çip yazıldı ama satır işaretlenemedi"* operatöre **"Plaque encoded."** görünür; istemci hazır, tel taşımıyor.

### 2026-08-25 (17. oturum, üçüncü yarı) — 🟢 **PUSH + DEPLOY: CANLI `af8b44c`, migration 22 ÜRETİMDE**

**Kullanıcı *"push yap deploy alınsın"* dedi.** Yedi commit push edildi, ve **CI kırmızı çıktı** — `TestAuditIndex00021_PlaqueHistoryCanSeekOnTheIndex` *(bu turda yeniden adlandırıldı; eski adı artık yok)*. 🔴 **Ve bu bir flake değildi: main 2026-08-22'den beri kırmızıydı, AYNI testle** (`32570619152` · `32856990423`), yani **üç gündür hiçbir şey deploy edilmiyordu ve bunu kimse fark etmemişti.**
