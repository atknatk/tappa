...
**Ne kaldı.** 🔴 **GERÇEK ÇİPTE step 8'i koşmak** — geri-dönülemez (çip fabrika anahtarını kaybeder), donanım kullanıcıda, kullanıcı kararı. Bu yapılınca çip GERÇEKTEN duvara çıkabilir. SIRADAKİ **İŞ 3 — Q02** (e-posta). ⚠️ md.5 dalı (`md5-key0-rotation`) T73'ün ucunda stacked; **main'e merge edilmedi** (merge sırası: T73 → md.5, kullanıcı deploy kararı). ⚠️ pre-existing sapma: `cmd/rotatekek` GOPACKAGESDRIVER (T72 sınıfı, backlog). 🔴 canlı DB şifreleri + Ozcan parolası (`Qw123654.!`) → ROTATE.
...
**Ne kaldı.** SIRADAKİ **İŞ 3 — Q02** (e-posta, seam hazır, 🔴 **sağlayıcı kullanıcı kararı bekliyor** — öneri AWS SES eu-central-1, ADR gerekli, §1 yeni bağımlılık ÖNCE SOR). Sonra İŞ 4 — M9-08 (operatör paneli, pilot sonrası). ⚠️ T73 dalda, **main'e merge edilmedi** — kullanıcı deploy kararı verince. 🔴 **Kullanıcıya borçlu:** canlı DB şifreleri + Ozcan panel parolası (`Qw123654.!`) sohbette açık → ROTATE · md. 5 (anahtar 0) 25. oturumda KOD olarak kapandı (kalan: gerçek çipte step 8) · üretim tenant'ında test verisi kalıcı · iki owner hesabı.
