...
| M8-05 B2c-2 | Encode aracı — **HTTP rölesi, yetkilendirme, kalıcılık** | todo | **donanım GEREKMEZ** · B2c-1'in portlarına gerçek uygulama: uç nokta + **`RequireAdmin`** + oran sınırı (**§6 md.10**) · `tags` satırının yazımı ve **`audit_log` olayı** (**md.8** — ad/aktör/tenant **hiçbiri kararlaşmadı**) · **md.9** `aes_key_ref` sütun kısıtı → **T16** (⚠️ eski çözümü **uygulanamaz**) · **md.12** UID işgali. 🔴 **İKİ BLOKLAYICI ŞART, ikisi de denetimden:** `Rows.InsertUnassigned` **`tenant_id`'yi AÇIK PARAMETRE** almalı — `SET LOCAL`'e örtük bırakılırsa §4.5'in kuşağı (açık filtre) **tamamen uygulamanın hafızasına** kalır · ve `tags`'te **"encoded" sütunu YOK**, `status` `unassigned` kalmak zorunda. ⚠️ **`keyring` `st.mu` ile korunmuyor** — sahibi `s.busy`'yi tutan goroutine'dir; bir *"oturum ne tutuyor"* sağlık ucu `st.mu` alıp halkayı okursa **canlı anahtar üstünde veri yarışı** olur (denetçi `-race` ile üretti) |
