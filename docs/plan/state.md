...
**Son güncelleme:** 2026-08-14 (11. oturum — **M7-04 FAZ B DONE → M7-04 KAPANDI · M7-07 AÇILDI · migration YOK · sıradaki M7-05**)
...
| **Sıradaki görev** | **M7-05 — Hesap ve marka mesajı ayarları.** Firma bilgileri · fatura verileri · plan görünümü · zaman dilimi (Q01) · marka mesajları (düzenleme editörü M9-04) · her değişiklik `audit_log`'a. ⚠️ **KART ÜÇ ŞEYİ BİRLİKTE İSTİYOR ve sebebi M7-02'de yazıldı:** `tenants.vat_verified` + `vat_checked_at` **hiçbir panel ekranı tarafından okunmuyor** — kayıt sihirbazının onay ekranı müşteriye bunu söyleyen **tek** yer ve bir kez gösterilip siliniyor, yani VIES'in **reddettiği** bir numara DB'de `false` duruyor ve müşteri onu bir daha göremiyor. Sütunları okuyan sorgu · ekranda görünen satır · ve **00017'de bilerek verilmemiş `UPDATE (vat_verified, vat_checked_at)` grant'i** (kimin ne sıklıkla tetikleyebileceği bu kartın kararı — müşterinin istediği zaman tetiklediği bir dış çağrı **farklı bir tehdit modeli**). ⚠️ Onay ekranının metni `TestSignupDone_PromisesNoPanelSurfaceForTheVATCheck` ile ürüne bağlı: bir sorgu sütunu okumaya başladığı gün kısıt **kendiliğinden** kalkıyor. 🔴 **VE M7-07 AÇILDI, SEBEBİ ÖLÇÜM:** bugün bir işletmenin **tam olarak bir** yöneticisi olabiliyor (`CreateAdminUser`'ın tek üretim çağrı yeri `signup.go:719`, `role: "owner"`) → `manager` rolü **üründe ulaşılamaz**, policy motorunun `actor:role` ayrımı **atıl**. Q02'ye bloke (davet davetlinin kendi adresine gitmeli). **⚠️ ÖLÇÜM ZEMİNİ (2026-08-14, `01a07bc`):** `make test` **3389 PASS / 0 FAIL / 0 SKIP · 20 paket**, duvar **~250 s** (`internal/handler` ~243–247 s **bilinçli ve kabul edilmiş — yeniden açma**); **çıplak `go test` DB testlerini SESSİZCE atlar (450 SKIP)** — **SKIP sayısını HER ZAMAN yazdır**; **`.templ` sonrası `make fmt gen`**; **her sayıya `uptime`**. `make audit` **exit 2 BEKLENEN** (govulncheck → **T31, kullanıcının işi**); `redline-check.sh` tek başına exit 0, **00020 yüzünden her koşuda R5 WARN**. Son migration **00020**, son ADR **0016**. **Canlı iddia olarak sayı/tarih yazma.** |
...
| M7-07 | **Admin daveti — bir işletmenin ikinci yöneticisi** *(M7-04 B kapanışında ölçülüp ayrıldı, 2026-08-14)* | todo | 🔴 **Bugün bir işletmenin TAM OLARAK BİR yöneticisi olabiliyor.** `CreateAdminUser`'ın tek üretim çağrı yeri `signup.go:719` ve `role: "owner"` yazıyor (grep) — yani `admin_users.role` sözlüğünün **`manager` değeri üründe ulaşılamaz**, ve onunla birlikte policy motorunun `actor:role` ayrımı, M6-09'un rol kapısı, M6-12'nin owner-only faturası **gerçek müşteride atıl**. Bloke: **Q02** (davet, davetlinin **kendi** adresine gitmek zorunda; taşıyıcı yok → bugün yapılırsa **ikinci bir ölü özellik** sevk edilir) |
...
**Özet:** **86 görev** · done **68** · **wip 0** · blocked 0 · skipped 1 · todo **17** · **M0+M1+M2+M3+M4+M5+M6 TAMAM 🎉🎉 · M7 5/7** *(2026-08-14: M7-04 B kapanışında **M7-07** açıldı — kartın başlığı "Admin daveti" diyordu, beş kabul kriterinin hiçbiri istemiyordu, ve ölçüm gösterdi ki **hiç yapılmamış**; 85'ten 86'ya)* *(2026-08-14: kullanıcı kararıyla **M7-06** ve **M9-08** açıldı → 83'ten 85'e)* — **ürünün fonksiyonel boşluğu kapandı: artık herkes kayıt olabiliyor** *(M5-11 M5-09'da bulunan §5 ihlali için kullanıcı kararıyla açıldı → toplam 82'den 83'e)*
