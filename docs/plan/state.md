...
**Son güncelleme:** 2026-08-06 (5. oturum — **M6-03 done · M6 3/12**)
...
| **Sıradaki görev** | **M6-04** — FLAGGED onay kuyruğu. **Kırmızı çizgi: §4.3** (`transactions` immutable → düzeltme = **yeni kayıt + `audit_log`**). **UI → skill `tappa-brand` ZORUNLU.** M6-03 okuma yolunu bıraktı: `internal/domain/ledger/` · `ListPanelTransactions` (16 sütun, **koordinat yok**) · docket kartı · filtre çubuğu · keyset sayfalama · HTMX (2.0.10, gömülü, digest **testle** doğrulanıyor) · genişletilmiş CSP **tam 1 bölümde**, kardinalite pinli. 🔴 **M6-04'ÜN BİLMESİ GEREKEN ÜÇ ŞEY:** (1) **`audit_log` YOLU HÂLÂ YOK** — ölçüldü (M5-11): `channel='manual'` **408 satır**, bunları hedefleyen `audit_log` **0**, manuel giriş HTTP rotası **YOK**; §4.3'ün *"düzeltme = yeni kayıt + `audit_log`"* yarısı **bu görevin işi** (M6-08 manuel girişi yazar, bu görev **onay/red** akışını). (2) 🔴 **KUŞAK AĞININ TÜRETİMİ ARTIK TÜM PAKETİ TARIYOR** (`*.go` glob, `_test.go` hariç) — M6-03'te tek dosya okuyordu ve bir dosya öteki kapsamsız sorgu **yeşil** kalıyordu; **bu pakete okuma/yazma eklerken ağın seni gördüğünü mutasyonla doğrula**. (3) `ledger.PageSize = 25` **bütçenin gerçek kolu** (kullanıcı kararı 2026-08-05: 25 kalsın; p99 gerçek gün **2 istek**). ⚠️ Ayrıca `store.AdminUser.PasswordHash` (sqlc **üretimi**) hâlâ çıplak `string`; bugün üretimde **sıfır referansı** var ama onu döndüren bir sorgu yazılırsa `%+v` sızdırır. |
...
| `make test` | **15 paket** `ok`, **PASS 1698 / SKIP 0 / FAIL 0** (M6-03 sonrası) · **gözlenen aralık 92–165 sn** (makine durumuna göre; **hedef değil, gözlem kaydı**) · sayım: `make test GOFLAGS=-v \| grep -c -- '--- PASS:'` · ⚠️ çıplak `go test` DB testlerini **sessizce SKIP eder** (ölçüldü: PASS 1352 / SKIP 221) |
...
**Özet:** 83 görev · done **55** · **wip 0** · blocked 0 · skipped 1 · todo **27** · **M0+M1+M2+M3+M4+M5 TAMAM 🎉 · M6 3/12** *(M5-11 M5-09'da bulunan §5 ihlali için kullanıcı kararıyla açıldı → toplam 82'den 83'e)*
