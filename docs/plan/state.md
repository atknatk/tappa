...
| | |
|---|---|
| **🟢 GÜNCEL (27. oturum, 2026-09-25) — aşağıdaki satırlar tarihsel** | **M10 — [Platform](m10-platform.md) Faz 0**, dal **`m10-faz0`** (origin'de; `main` = `b09ba1e`, canlı `aefa67d` içeriği). ✅ F0-5 `ea43e4d` · F0-7 `3844ea1` · OP-2 `30284ce` · WL-5 atıf düzeltmesi `e226eea` (main CI'ı yeşile döndürür). ⏳ **Sıradaki:** F0-6 (yapıcıda) → OP-3 (migration **00024**, tappa-db-migrator) → Faz 0 üçüncü göz + güvenlik → **main'e birleştirme = deploy = KULLANICI KARARI** → A1 (OP-4: ADR **0020** + **0021**). **Kullanıcıda:** F0-1 panel parolası rotate · F0-4 DB parolaları rotate · Rusty Bar plaketi Replace · SES dış adımları. **Sır kapısı:** push öncesi kanca bu makinede kurulu (`make hooks`); push daima uzağın ADIYLA (`git push -u origin <dal>`). |
