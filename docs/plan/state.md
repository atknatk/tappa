...
**Son güncelleme:** 2026-08-17 (13. oturum — **M8-02 FAZ F DONE** (`a32d0ca`, **17 tur / 9 RED**, dokuz farkli ucuncu goz + dort guvenlik denetimi) · KEK dondurme araci + iki-KEK okuma yolu + `scripts/rotate-kek.sh` sevk edildi · **Docker Hub kurulumu KULLANICI TARAFINDAN yapildi ve dogrulandi** · sıradaki **deploy'un yesile donmesi**, sonra M8-03)
...
| **Sıradaki görev** | **DEPLOY'UN YESILE DONMESI** — `a32d0ca` push edildi, CI→deploy kosuyor. **Uc blokaj 2026-08-17'de kapandi:** `DOCKERHUB_USERNAME`+`DOCKERHUB_TOKEN` GitHub sirri **yazildi** (Docker Hub login API'siyle dogrulandi: **HTTP 200, JWT dondu, 2FA engeli yok**), ve `atknatk/tappa` + `atknatk/tappa-migrate` **Public olarak yaratildi** (ikisi de 404'tu; `is_private=false`, anonim GET ile dogrulandi). 🔴 **KALAN BILINEN RISKLER, deploy kirmiziya donerse ILK BAKILACAKLAR:** (1) `deploy/k8s/01-rbac.yaml` **kumeye hic uygulanmadi** — canli `Role/github-deployer` `secrets`'a **yedi fiil** veriyor ve **`pods/exec` YOK**, yani 19. adimdaki `db-role` kapisi **yine fail-closed dusebilir** (backlog **T44**). Cure tek komut, **cluster-admin kubeconfig ile, bir kez**: `kubectl apply -f deploy/k8s/00-namespace.yaml` sonra `-f deploy/k8s/01-rbac.yaml` (**apply**, asla delete+create). (2) `TAPPA_TAG_KEK_PREVIOUS` sirda **yok** ve `optional: true` bunu **sessizce yutuyor** — bu **dogru** (surekli durum), ama rotasyon penceresi acilirken sirra eklenmeli. (3) Yedek CronJob'i kumede **YOK** (`get cronjob` → `No resources found`) → backlog **T45**. **Sonraki gorev M8-03** (gozlemlenebilirlik), M8-02'nin kalan kriterleri **sayilmis limit** olarak kapali: *"managed Postgres"* (kullanici karari), **DPA/Q23** (hukuki, B3), *"deploy penceresi vardiyalara gore"* (soguk node cekme suresi **olculmedi ve olculmedigi yazili**). |
