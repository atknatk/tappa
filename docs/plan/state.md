...
**Son güncelleme:** 2026-08-09 (7. oturum — **M6-06 A + B VERİ KATMANI DONE · M6-06 wip · M6 5/12**)
...
| **Sıradaki görev** | **M6-06 B — UYGULAMA KATMANI** (panel: plaket listesi · replace tag · tag geçmişi · *encoded/pending*). **Veri katmanı KAPANDI** (`ba671b0`, migration **00013**). **Kırmızı çizgi: §4.7** (AES anahtarı ekranda **HİÇ**) **ve §4.4**. **skill `tappa-sun` ZORUNLU** · **skill `tappa-brand` ZORUNLU** · **yeni migration YOK** (00013 B'nin yuvasıydı ve harcandı — ihtiyaç doğarsa **bana sor**). 🔴 **VERİ KATMANININ DEVRETTİĞİ ON YÜKÜMLÜLÜK** (hepsi ölçülmüş): **(1)** `internal/sun/preview.go:179` monte edilmemiş/emekli plakette *"imza doğrulanamadı"* diyor — **sorun durum, imza değil**; doğru sonuç, **yanlış gerekçe**. **(2)** `ResolvedTag.LocationID` **`uuid.UUID`, işaretçi değil**; pgx NULL'ı **hatasız `uuid.Nil`** yapıyor (pinli v5.10.0 ile ölçüldü) ve bu **beş elle yazılmış resolver'ın tek nullable sütunu** — anlamı yorumda, **tip hâlâ tekil**. **(3)** retired/unassigned plakete tap **HTTP seviyesinde testsiz** (guardrail seviyesinde pinli). **(4)** stok plaketin **panelde görünümü** — `ListTagsForTenant` stoku **başa** koyuyor (`NULLS FIRST`), ekran dili kararı verilmedi. **(5)** `ListTagLastSeen` liste ile **Go tarafında** birleştirilmeli — *"hiç taplanmadı"* **anahtarın yokluğu**, sıfır zaman **değil** (sqlc dört şekilde de yalan söylüyor; cast'li hâli **tam da bu görevin eklediği satırlarda** çöküyor). **(6)** **44 baytlık zarf zorlanmıyor** (backlog **T7**) — tek savunma `sun.Unwrap`'ın uzunluk kontrolü, yani **500**. **(7)** replace tag'in **30 sn** bütçesi ölçülmedi; retire + assign **tek transaction** olmalı, yoksa *"eski emekli, yeni bağlanmamış"* ara durumu mümkün. **(8)** `@replaced_by` **handler sınırında doğrulanmalı** — SQL artık sessiz kırpmıyor (`ErrNoRows`) ama §7 doğrulaması **borç**. **(9)** plaket **yükleme yolu `tappa_owner`'ın** (M8-05); `tags` INSERT'i `tappa_app`'te **DURUYOR** (7 test dosyası kullanıyor) → **T8'in *bugün sömürülemiyor* koruması bu fazda BİTMİYOR, bir yükleyici yazıldığı gün bitiyor.** **(10)** `ledger`'ın `stopRE`'sinde `RETURNING` yok — **iki yönde ölçüldü, ATIL**; kapatmak kendi mutasyon turunu ister. ⚠️ **VERİ KATMANI ÜÇ DEVRALDIĞIMIZ CÜMLEYİ ÇÜRÜTTÜ:** T8'in *"mevcut satırlar zaten büyük harf, veri taşıması yok"*u (**18.010 küçük harfli**, 12.437'si **24.874** transaction'dan referanslı → §4.3) · T9'un trigger koşulu (`> OLD` **retire'ı reddediyor**) · ve T9'un **sütun listesi** (`location_id` yoktu, envanter modeli onu **zorunlu** kılıyor). **Kart ve backlog düzeltildi.** **A FAZININ + VERİ KATMANININ KALIPLARI:** mutasyon rotası `ProtectWriting` (**dört aşama, SAY**) · audit **`RecordTx` ile aynı transaction'da** · geri alınamaz eylem → **MAC'li onay v2** (`action|subject|session`, **on iki** parça) **artı `owner`-only** · bildirim → **C′** (aktörün audit satırına karşı **aynı istekte** doğrula, **asla `Done`'a bağlama**) · kapalı sözlük **türetilir** · ve **kuşak ağı artık DOSYADAN türetiyor** (çağıran gerekmiyor) — bugün **24/65 (%36,9)**, ⚠️ **canlı iddia olarak yazma**. |
...
**Özet:** 83 görev · done **57** · **wip 1** (M6-06 — **A fazı done**, B açık) · blocked 0 · skipped 1 · todo **24** · **M0+M1+M2+M3+M4+M5 TAMAM 🎉 · M6 5/12** *(M5-11 M5-09'da bulunan §5 ihlali için kullanıcı kararıyla açıldı → toplam 82'den 83'e)*
