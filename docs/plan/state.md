...
**Son güncelleme:** 2026-08-16 (12. oturum — **M8-02 FAZ C** (`0ce5615`, 11 tur/5 RED) ve **FAZ D** (`1b2d623`, 9 tur/5 RED) **DONE** · **FAZ E denetimde, commit edilmedi** · T41/T42/T43/T44 kapandı, **T45/T46 açıldı** · sıradaki **FAZ E'nin kapanışı, sonra FAZ F (KEK aracı)**)

> ⚠️ **BU DOSYADAKİ ESKİ BLOKLARI OKURKEN:** aşağıdaki 2026-08-16 blokları **yazıldıkları anın** durumunu anlatıyor ve bir kısmı **sonraki fazlarda çürütüldü** — her biri kendi geri çekmesini taşıyor. **Bugünkü durum için YALNIZCA ledger satırına bak** (`M8-02` satırı) — ⚠️ *"ŞU AN"* tablosunun **"Sıradaki görev"** hücresi de bayat: FAZ E denetimden geçti, ve o hücre emekli `[yedek YOK]` sınır anahtarını hâlâ kullanıyor (bugünkü adı **`[yedek kümede HENÜZ YOK]`**). **Bir uyarı, kapatmak için var olduğu kusuru kendi işaret ettiği hedefte bırakamaz** — bu satır tam olarak o yüzden düzeltildi. Özellikle: *"paketleri public yap ya da PAT'i `ghcr` sırrına yaz"* reçetesi **FAZ D'de ÖLDÜ** (registry **public Docker Hub**'a taşındı, `secret/ghcr` ağaçtan **kaldırıldı**) — backlog **T43** kapanışı bunu ölçümüyle yazıyor. Ve *"FAZ D'ye kalan: yedek + provalı geri yükleme"* → **FAZ E'nin işiydi, yapıldı, ama kümede YOK** (backlog **T45**).
...
| **Sıradaki görev** | **M8-02 FAZ F — KEK DÖNDÜRME ARACI** (yedek FAZ E'de yapıldı, denetimden geçti, **commit bekliyor**; kümede yok → backlog **T45**). ⚠️ **AMA ÖNCE ÜÇ KULLANICI EYLEMİ — ÜÇÜ DE DEPLOY'U TAMAMEN BLOKLUYOR** (aşağıdaki "Blokeler" satırı). **FAZ D koştu ve iki iş yaptı: T44 (RBAC) + T43 (registry).** Yedek ve KEK aracı **yapılmadı** — diff'te `0` adet `.go`, `kubectl -n tappa get cronjob` → **No resources found**. 🔴 **VE FAZ D'NİN SONUCU BİR CÜMLEYLE: DEĞİŞİKLİKLER AĞAÇTA, KÜME HÂLÂ ESKİ.** Ölçüldü (denetçi, 2026-08-16): kümede `secret/ghcr` **VAR**, **üç ReplicaSet'in üçü de** `ghcr.io/...` + `imagePullSecrets: ghcr`, canlı `Role/github-deployer` `secrets`'a **yedi fiil + `resourceNames` yok** ve `pods/exec` **yok**. Yani `TAPPA_TAG_KEK` bugün hâlâ deploy kimliğinin patlama yarıçapında ve `db-role` kapısı sonraki koşuda **yine düşer**. **FAZ E'nin işi:** **(1) YEDEK + PROVALI GERİ YÜKLEME** — kartın cümlesi *"otomatik, geri yükleme **denenmiş** (denenmemiş yedek yedek değildir)"*; bugün yedek **yok**, sınır **[yedek YOK]** bunu sayıyor (`local-path`, **tek node**, `reclaimPolicy: Delete`). 🔴 **M8-06 pilotu başlamadan kapatılmalı** — pilot bir haftalık **paralel kayıt** üretiyor ve o hafta **§4.3 gereği yeniden kurulamaz**. **(2) KEK DÖNDÜRME ARACI** — *"tüm parkın `tags.aes_key_ref` değerlerini yeniden sarmalayan araç"*; repoda **yok** (`internal/sun/keys.go` `Wrap`/`Unwrap` var, `Rewrap`/`Rotate` **yok**), yani **bugün bir KEK sızıntısının yürütülebilir karşılığı yok** ve `TAPPA_TAG_KEK` kaybı parktaki **her plaketin fiziksel olarak yeniden encode edilmesi** demek. ⚠️ Araç §4.7 **ve** §4.5'e birden değiyor: anahtar log'a **asla** yazılmaz, iş **tenant kapsamlı** RLS altında koşar, ve **yarıda kesilirse karışık sarmalı bir park bırakmamalı** — eski ve yeni KEK'in **bir arada okunabildiği** bir geçiş penceresi **tasarım kararıdır**, sorulmadan varsayılmasın. **Araç:** `tappa-db-migrator` + skill `tappa-sun`. 🔴 **VE İLK ADIM YİNE KARTI ÖLÇMEK** — bu oturumda **beş kez** işe yaradı: T41'in *"yarış değildi"*i, T42'nin *"`kubectl wait` yok"*u, T43'ün *"sıralama"*sı, T44'ün *"üç nesne"*si (gerçek **beş**) ve *"paketleri public yap"* reçetesi, **beşi de** yazılı olduğu hâlde yanlıştı. ✅ **ÖNCE ŞUNU BİL: FAZ C KÜMEDE DOĞRULANDI.** `a7330ae` deploy'u migration'ı **ilk kez** geçirdi — `job/tappa-migrate` **`Complete 1/1`, 8 sn** (önceki koşu: `Failed`, 383 sn), yeni sunucu pod'u **`1/1 Running`, `RESTARTS=0`**, `/healthz` ve `/readyz` **200**, ve **eski pod'un bayat `ghcr` kimliği sorunu da kendiliğinden kalktı** (yeni pod taze sırla çekti). Küme artık manifestlerin tarif ettiği şey. 🔴 **AMA DEPLOY 19. ADIMDA KIRMIZI VE SEBEBİ YENİ BİR BULGU → backlog T44** — `db-role` kapısı `pods/exec` yetkisi olmadığı için koşamıyor ve **fail-closed** düşüyor (*"NOTHING was checked. This is not a pass."*). ⚠️ **Asıl mesele kapının kırmızı olması değil: o kapı ÜRETİMDE HİÇ KOŞMAMIŞ** — `Role/github-deployer` `pods`'a tüm fiilleri veriyor ama `pods/exec` **ayrı bir alt kaynak** ve listede yok; önceki her deploy migration'da düştüğü için 19. adıma **hiç ulaşılmamıştı**. **Bu oturumun imza sınıfının altıncı örneği.** Ve daha ağırı: `Role`/`RoleBinding`/`ServiceAccount` **yalnız kümede var, repoda yok** (elle yaratıldı `2026-08-15T20:15:45Z`), yani taze bir küme deploy'u hiç koşturamaz, rolü **hiçbir denetim görmedi**, ve rol `ns/tappa`'da `secrets`'a **tam CRUD** veriyor — **`TAPPA_TAG_KEK` dahil** (§4.7). 🔴 **ELLE YAMAMA:** T41/T42/T43'ün üçü de *"kümede elle düzeltildi, manifest bilmiyor"* diye doğdu. **Sonra kartın kalan İKİ YÜRÜTÜLEBİLİR kriteri.** FAZ C `0ce5615` ile kapandı (11 tur / 5 RED, iki denetçi ONAY). **(1) YEDEK + PROVALI GERİ YÜKLEME** — kartın cümlesi *"otomatik, geri yükleme **denenmiş** (denenmemiş yedek yedek değildir)"*. Bugün **yedek YOK** ve `deploy/README.md` sınır **[yedek YOK]** bunu sayıyor: `local-path`, **tek node**, `reclaimPolicy: Delete`, replika yok, `kubectl -n tappa get cronjob` **boş**. 🔴 **M8-06 pilotu başlamadan kapatılmalı** — pilot bir haftalık **paralel kayıt** üretiyor ve o hafta **§4.3 gereği yeniden kurulamaz**. **(2) KEK DÖNDÜRME ARACI** — *"KEK dönme prosedürü yazılı **ve yürütülebilir**: tüm parkın `tags.aes_key_ref` değerlerini yeniden sarmalayan araç var"*. Repoda **yok** (ölçüldü: `internal/sun/keys.go` `Wrap`/`Unwrap` **var**, `Rewrap`/`Rotate` **yok**; `cmd/` altında tek binary), yani **bugün bir KEK sızıntısının yürütülebilir karşılığı yok** ve `TAPPA_TAG_KEK` kaybı parktaki **her plaketin fiziksel olarak yeniden encode edilmesi** demek. ⚠️ **Araç §4.7'ye ve §4.5'e birden değiyor:** anahtarlar log'a **asla** yazılmaz, iş **tenant kapsamlı** ve RLS altında koşar, ve yarıda kesilirse **karışık sarmalı bir park** bırakmamalı — yani eski KEK ile yeni KEK'in **bir arada okunabildiği** bir geçiş penceresi tasarım kararıdır, sorulmadan varsayılmasın. **Araç:** `tappa-db-migrator` (yeni sorgu gerekirse) + skill `tappa-sun`. ⬜ **Kartın karşılanmayan diğer üçü FAZ D'nin işi DEĞİL, kayıt için:** *"managed Postgres"* (kullanıcı kararıyla düz Postgres) · **DPA/Q23 + saklama süresi** (hukuki, backlog **B3**) · *"deploy penceresi vardiyalara göre"* (`maxSurge:1/maxUnavailable:0` sevk edildi; **soğuk node'da çekme süresi ölçülmedi ve ölçülmediği yazılı**). 🔴 **VE İLK ADIM YİNE KARTI ÖLÇMEK** — bu oturumda üç kez işe yaradı: T41'in *"yarış değildi"*i, T42'nin *"`kubectl wait` yok"*u ve T43'ün *"sıralama"*sı, **üçü de** yazılı olduğu hâlde yanlıştı. |
...
**Özet:** **86 görev** · done **70** · **wip 1** (M8-02 — FAZ A/B/C done, **FAZ D açık**) · **blocked 1** (M7-07, Q02) · skipped 1 · todo **13** · **M0+M1+M2+M3+M4+M5+M6 TAMAM 🎉🎉 · M7 6/7 — kalan tek görev Q02'ye bloke, yani M7 KULLANICI DIŞINDA BİTTİ · M8 1/7 done + 1 wip** *(2026-08-14: M7-04 B kapanışında **M7-07** açıldı — kartın başlığı "Admin daveti" diyordu, beş kabul kriterinin hiçbiri istemiyordu, ve ölçüm gösterdi ki **hiç yapılmamış**; 85'ten 86'ya)* *(2026-08-14: kullanıcı kararıyla **M7-06** ve **M9-08** açıldı → 83'ten 85'e)* — **ürünün fonksiyonel boşluğu kapandı: artık herkes kayıt olabiliyor** *(M5-11 M5-09'da bulunan §5 ihlali için kullanıcı kararıyla açıldı → toplam 82'den 83'e)*
