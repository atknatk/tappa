...
**Son güncelleme:** 2026-08-02 (5. oturum — **M5-01…M5-10 done, M5 10/11 · geriye yalnız M5-11 kaldı**)
...
| | |
|---|---|
| **Kilometre taşı** | **M0 + M1 + M2 + M3 + M4 TAMAM** ✅ · **M5 — [Tap akışı](m5-tap-akisi.md) 10/11** · **🎉 uçtan uca check-in ÇALIŞIYOR**, **iki kanalda** (davet → aktivasyon → mini tur → **NFC veya QR** → karar → kayıt → onay ekranı) ve artık **gerçek HTTP + gerçek Postgres üzerinde bir GÜN** olarak kanıtlı (`make simulate-day` — 10 çalışan, 31 kayıt, kayıtları **karar motoru** üretiyor) · tap sayfası artık **3 dk'lık tazelik penceresine** bağlı |
| **Sıradaki görev** | **M5-11 — M5'in SON görevi** ([Practice satırı açık girişi maskeliyor](m5-tap-akisi.md#m5-11--practice-satırı-açık-girişi-maskeliyor-5-yön-kuralı-ihlali)). **Sevk edilmiş kodda bir §5 ihlali** ve kullanıcı kararıyla açıldı (2026-08-02). Kart üç tasarım seçeneğini, üç sondanın ölçümünü ve gerçek-hayat senaryosunu (kayıp telefon → yeniden aktivasyon → practice satırı açık girişi maskeliyor) taşıyor. **Semantik karar → ADR ZORUNLU** (CLAUDE.md §10), ve ADR *"bugün yazılmış, kapanmamış açık kayıtlara ne olacak"* sorusunu da cevaplamalı (§4.3: düzeltme = **yeni kayıt + `audit_log`**, UPDATE değil). **Bitiş şartı:** M5-09'un gün testindeki **workaround kaldırılacak** (`day_db_test.go` Ivan'ın 17:50 beyanı) — kaldırılmazsa görev bitmemiştir; ayrıca `LIMITS L3` ve M5-09 kart md. 6 kapatılacak. **Sonra M5 kapanır → M6.** |
...
| `make test` | 13 paket `ok`, **PASS 1289 / SKIP 0 / FAIL 0** (M5-10 sonrası) · **~85–142 sn** · sayım: `make test GOFLAGS=-v \| grep -c -- '--- PASS:'` |
...
**Özet:** 83 görev · done 51 · wip 0 · blocked 0 · skipped 1 · todo 31 · **M0+M1+M2+M3+M4 TAMAM · M5 10/11 · sıradaki M5-11 (M5'in son görevi)** *(M5-11 M5-09'da bulunan §5 ihlali için kullanıcı kararıyla açıldı → toplam 82'den 83'e)*
