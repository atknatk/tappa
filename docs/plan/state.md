...
**Son güncelleme:** 2026-08-15 (11. oturum — **M7-04 KAPANDI · M7-05 DONE · M7-07 AÇILDI (Q02'ye bloke) · migration YOK · 🎉 M7 KULLANICI DIŞINDA BİTTİ · sıradaki M8-01**)
...
| **Sıradaki görev** | **M8-01 — Paketleme.** 🎉 **M7 KULLANICI DIŞINDA BİTTİ** (6/7; kalan M7-07 **Q02'ye bloke**), yani ürünün hem çalışanın hem müdürün hem de kayıt olanın gördüğü tarafı tamam ve sırada **üretime çıkarmak** var. Kart (`m8-deploy-pilot.md` → M8-01): `make build` → **`CGO_ENABLED=0`**, statik, **`-trimpath`**, sürüm/commit gömülü · statik varlıklar ve şablonlar **binary içinde** (`web/embed.go`), runtime dosya bağımlılığı **yok** · migration **ayrı adım** (`tappa_owner`), başlangıçta **otomatik migrate YOK** (yanlış rolle çalışma riski) · `/healthz` (canlı) + **`/readyz`** (DB'ye bağlı) · **`time/tzdata` gömülü**. ⚠️ **SON MADDEYİ HAFİFE ALMA — bu oturum onun bedelini iki kez ölçtü:** `fadd9a7` `make check`'i her gün iki saat kırmızı yapan bir zaman-dilimi çözümü kusurunu kapattı, ve M7-05'in ikinci bloklayanı **müşterinin kendi kayıt tarihinin UTC'de basılmasıydı**. `time.LoadLocation` tzdata'sız bir imajda **hata döndürür**, ve bu ürünün **on bir müşteri-görünür tarihi** ile **vardiya/geç kalma hesabının tamamı** oradan geçiyor — yani eksik tzdata sessiz bir yanlışlık değil, **§5'in geç kalma kuralını topyekûn UTC'ye düşüren** bir arıza. **Gömülü olduğunu bir testle kanıtla**, gömdüğünü varsayma. ⚠️ **VE İKİ KART MADDESİNİN BUGÜNKÜ DURUMUNU ÖNCE ÖLÇ:** `web/embed.go` ve `/readyz` **var mı**? (M7-03'te dört kriterin üçü, M7-05'te dördün biri **zaten sevk edilmişti** — kartı ölçmek bu projede üç kez işe yaradı.) 🔴 **VE M8 GÜVENLİK BORÇLARININ ADRESİ:** backlog **T28** (HSTS ürün genelinde YOK) · **T29** (`HEAD` 405 veriyor, `/healthz` dahil — **bu kartın `/healthz` maddesiyle doğrudan çakışıyor**, uptime monitörleri HEAD'ler) · **T30** (`TAPPA_TRUSTED_PROXIES` boşken panel giriş bütçesi tek adrese çöker — **dağıtım belgesinde zorunlu adım**) · **T27/T37** (faturalama, pilot öncesi) · **T31** (`go1.26.6`, kullanıcının işi). **⚠️ ÖLÇÜM ZEMİNİ (2026-08-15, `145b344`):** `make test` **3469 PASS / 0 FAIL / 0 SKIP · 20 paket**, `make check` **exit 0** temiz ağaçta **246 s** (load 1,87→2,75); **çıplak `go test` DB testlerini SESSİZCE atlar (450 SKIP)** — **SKIP sayısını HER ZAMAN yazdır** (bu oturumda **iki denetçi** o tuzağa düştü, biri mutasyon altında **`ok`** aldı çünkü dört DB testi SKIP oldu); **`.templ` sonrası `make fmt gen`**; **her sayıya `uptime` ve `date -u`**. `make audit` **exit 2 BEKLENEN** (govulncheck → **T31**); `redline-check.sh` tek başına exit 0, **00020 yüzünden her koşuda R5 WARN**. ⚠️ **8080 portunu bu repoyla ilgisiz bir konteyner tutuyor** (`lse-authz`) ve **`tappa-db` bu oturumda bir kez kendiliğinden durdu** — sondan önce `docker compose ps`. Son migration **00020**, son ADR **0016**. **Canlı iddia olarak sayı/tarih yazma.** |
...
**Özet:** **86 görev** · done **69** · **wip 0** · **blocked 1** (M7-07, Q02) · skipped 1 · todo **15** · **M0+M1+M2+M3+M4+M5+M6 TAMAM 🎉🎉 · M7 6/7 — kalan tek görev Q02'ye bloke, yani M7 KULLANICI DIŞINDA BİTTİ** *(2026-08-14: M7-04 B kapanışında **M7-07** açıldı — kartın başlığı "Admin daveti" diyordu, beş kabul kriterinin hiçbiri istemiyordu, ve ölçüm gösterdi ki **hiç yapılmamış**; 85'ten 86'ya)* *(2026-08-14: kullanıcı kararıyla **M7-06** ve **M9-08** açıldı → 83'ten 85'e)* — **ürünün fonksiyonel boşluğu kapandı: artık herkes kayıt olabiliyor** *(M5-11 M5-09'da bulunan §5 ihlali için kullanıcı kararıyla açıldı → toplam 82'den 83'e)*
