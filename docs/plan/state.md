...
**🔴 KALAN ÜÇ İŞ — sıradaki İŞ 2 (T73):**
· **İŞ 2 — T73 (parola değiştirme, küçük-orta, Q02'DEN BAĞIMSIZ, pilot-öncesi):** mekanizma HAZIR — `internal/adminauth/password.go` (`Hash` bcrypt cost 12, `Compare`), `signup.MinPasswordRunes=12`, `adminreset.go` `checkNewPassword`, `RevokeAdminSessionsForAdmin` (`admins.sql:175`). **EKSİK:** (a) `admins.sql`'e 2 sorgu — id+tenant ile `password_hash` OKU (mevcut parolayı doğrula) ve GÜNCELLE (`GetAdminByID` password_hash seçmiyor; migration GEREKMEZ) · (b) yeni handler `POST /admin/account/password` (`mountWriting`, ProtectWriting; desen: `accountactions.go:106` `accountSave`) · (c) Account formu (`account.templ:190`) · (d) testler (handler + DB/RLS + `-race`). **Karar K3:** parola değişince **diğer oturumlar iptal, mevcut hariç** (session id hariç `RevokeAdminSessionsForAdmin`). `tappa-db-migrator` (sorgular) + `tappa-brand` (form) + `tappa-security-auditor` (§4.7 auth).
