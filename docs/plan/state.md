...
**Son güncelleme:** 2026-09-12 (20. oturum — 🟡 **LANDING: KULLANICININ KENDİ TASARIMI İSKELETE TAŞINIYOR — WIP, DENETLENMEDİ, YARIN DEVAM.** Kullanıcı kendi yaptığı landing HTML'ini paylaştı ve *"bunu uygulayalım"* dedi. Dosya **verbatim uygulanamaz** (ölçüldü — aşağıda), kullanıcıya iki soru soruldu, iki karar alındı: **(1) Yaklaşım: tasarımı mevcut, 4-tur-denetimli iskelete TAŞI** — görünüm/yapı/enerji/ses kullanıcının dosyasından, iddia disiplini + palet + self-host font + gerçek rotalar mevcut sayfadan; kanıt/yargı bölümleri **korunur** · **(2) *"Piloting with Kebab Factory (9) / Kebab Manufacturing (5)"* şeridi pilot başlayana kadar YOK** (seed/demo tenant'ları; M8-06 başlamadı). **Referans dosya repoda:** `docs/design/landing-reference-2026-09-12.html` (`3214a5f`). · **Orkestratörün kendi kararlaştırdığı üç varsayım (kullanıcıya söylendi, itiraz gelmedi):** marka adı **Taptime** (dosya `Tappa` diyor; kullanıcının kendi kararıyla canlı site Taptime) · **plaket vaatleri YOK** (*"included & replaced free"*, *"stick a spare"* — plaketler encode bile edilmedi, M8-05 B3 donanıma bloke) · **SSS'deki QR cümlesi §5'e çekilir** (dosya *"IP **or** GPS"* diyor; §5: QR'da IP zorunlu, GPS tek başına `flag`, `base:qr-requires-ip`). · 🔴 **Dosyada TAŞINAMAYAN cümleler ve neden (yapıcı brief'ine tek tek yazıldı; yarın brief yeniden yazılırken buradan al):** **testlerin yakaladığı** — Google Fonts `<link>` (mutlak URL yasağı; fontlar self-host) · `tappa-portal.html`/`mailto:hello@tappa.mt` (`EveryInternalLinkResolves`; rotalar `v.SignupHref`/`v.SignInHref`) · *"under 10 minutes"*, *"30 seconds, ever"*, *"Under two seconds"*, *"about €90/month"*, *"one pay period"*, *"in ten seconds"*, *"A5"* (`MakesNoUnmeasuredNumericClaim`) · *"Retire the fingerprint box"*, ✗/✓ glifleri, *"less than one terminal's yearly maintenance"*, *"Site goes blind"* (`ComparesMechanismsAndDeclaresNoWinner`) · **mount edilmemiş yetenek (3 RED'i üreten sınıf), grep ile ölçüldü** — *"API for your payroll flow"* (`/api/*` yalnız tap formu POST'ları) · *"Import staff from CSV"* (yok) · *"live headcount per location"* (yok; headcount yalnız faturada) · *"Book a pilot"* mailto (iletişim kanalı yok, N-f) · *"Runs alongside… during trial"*, *"in view of your camera"* (özellik değil) · *"switched off… instantly / that second"* (zamanlama iddiası; mekanizmayı söyle) · *"IP + GPS have to agree"* (§5 satır 6 **AYRIK**: IP **veya** GPS) · `KF ST JULIANS · 91AC-7E55` (plaket UID şekli, kamuya açık sayfada yasak) · **marka skill'i** — 17 palet dışı hex · 12–24px yuvarlak kart/hap buton · 🏷️✉️📶🇲🇹 emoji · `backdrop-filter` · `infinite` animasyon · ağır gölge · **saffron metin olarak** (2,62:1) · **kaşe metni yeşil** (2026-08-01: kelime `ink`, renk çerçevede) · adisyon döndüren `<script>` (istemci state'i) · nav'daki yuvarlak yeşil nokta logo. **Taşınacaklar:** yapışkan nav + sayfa içi bağlantılar · hero sahnesi (koyu plaket + basılan adisyon, tek seferlik animasyon) · satış sesi (*"before the lunch rush"*, *"wet or floury hands"*, *"Nothing per device — there are none"*) · koyu `ink` ledger (mekanizma karşılaştırması + dipnot, ✗/✓ yok, metin yalnız token) · akordeon SSS (`<details>`) · yeşil CTA · fiyat kartı + **yalnız gerçek** onay listesi (panel+FLAGGED kuyruğu ✓ · CSV dışa aktarma ✓ · manuel kayıt ✓ · API ✗ · plaket ✗) · üstü çizili slogan fikri **ikincil** yerde (`<h1>` dört satır kalır, test pinli) · *"Security — four proofs"* ≡ mevcut *"Four pieces of evidence"* → **birleştirilir**, iki kez anlatılmaz. · **NEREDE KALDI:** yapıcı ajan ~1 saat çalışıp kullanıcı tarafından durduruldu (bilgisayar uykuya girecekti); iş **`6d6ddcb`'de WIP olarak commit'lendi** — nav (`landingChrome`, `NavLink`), hero sahnesi (`landingScene`), `<details>` SSS, fiyat, CTA, ve **yeni `internal/handler/marketing_facts_test.go`** (731 satır). Checkpoint ölçümü: `go build`/`go vet` temiz · `make gen` senkron · **38 landing testi PASS** (28 eski + 10 yeni) · `redline-check.sh` exit 0 (referans dosya dâhil). **ÖLÇÜLMEDİ:** `make check` uçtan uca · `make audit` · koyu zemin kontrastı · ölü CSS (base `bbeecc0` ile) · render sondaları · ekran görüntüleri · sihirbaz/legal regresyonu · **ve hiçbir dürüstlük invariant'ı** — yani yapıcının tamamladığı kısım bile henüz denetlenmedi. **YARIN:** (a) yapıcıyı `6d6ddcb`'den devam ettir — kalan: kontrast tablosu, ölü CSS ölçümü, `make check`+`make audit`, render kanıtı, ekran görüntüleri, ve **her yeni cümlenin `landingview.go`'da çapası** · (b) ekran görüntülerini kullanıcıya göster, yön al · (c) **taze üçüncü göz** — bu sayfanın 5. denetim turu; önceki 4'ün 3'ü RED ve hepsi *"ürünün yapmadığını söylemek"* sınıfından, kullanıcının dosyası bu sınıftan **çok** cümle taşıyor · (d) onay → commit'i WIP'ten temize çek (`6d6ddcb` mesajı *"NOT audited"* diyor, squash/reword) · `state.md` · dalı push (CI koşar, deploy **koşmaz**). ⚠️ **Uyku dersi:** ajanlar bu Mac'te yerel süreç; sistem uykusu/kapak/VSCode kapanışı öldürür (bu oturumda 2 kez yaşandı). Uzun tur başlatırken `caffeinate -dims -t 14400` koştur. · **Ledger DEĞİŞMEDİ** — "ŞU AN" hâlâ M8-05 FAZ B3; landing plansız kullanıcı işi, dal `landing-redesign`, `main` `40bc88c`'de, üretim etkilenmedi)
