...
| M8-05 B2c | Encode aracı — **durumlu oturum ve HTTP rölesi** | todo | **donanım GEREKMEZ** · AN12196 vektörleriyle uçtan uca doğrulanabilir · ADR 0017 §6'nın **on beş açık maddesi** kabul kriteri (özellikle **md.7**: oturum TTL + eşzamanlılık + **her çıkış yolunda `Zero`**; **md.9**: `aes_key_ref`'in sütun düzeyi INSERT kısıtı → **T16**; **md.10**: uç noktada **yetkilendirme kapısı yok**; **md.12**: UID işgali; **md.13**: `FileAR.Change`/`ReadWrite` kararı) |
