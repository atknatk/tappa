...
🔴 **KULLANICIYA BORÇLU:** **canlı `tappa_app` + `tappa_owner` DB şifreleri bu oturumun sohbetinde açığa çıktı → ROTATE.** İki owner hesabı (çift signup, dokunulmadı). Üretim tenant'ında test verisi kalıcı (Ozcan Test + tap kayıtları, §4.3). **md. 5** (anahtar 0 şeması — çip duvara çıkamaz) hâlâ açık.
...
**Ne kaldı.** SIRADAKİ **İŞ 3 — Q02** (e-posta, seam hazır, 🔴 **sağlayıcı kullanıcı kararı bekliyor** — öneri AWS SES eu-central-1, ADR gerekli, §1 yeni bağımlılık ÖNCE SOR). Sonra İŞ 4 — M9-08 (operatör paneli, pilot sonrası). ⚠️ T73 dalda, **main'e merge edilmedi** — kullanıcı deploy kararı verince. 🔴 **Kullanıcıya borçlu:** canlı DB şifreleri + Ozcan panel parolası (`Qw123654.!`) sohbette açık → ROTATE · md. 5 (anahtar 0 — çip duvara çıkamaz) hâlâ açık · üretim tenant'ında test verisi kalıcı · iki owner hesabı.
