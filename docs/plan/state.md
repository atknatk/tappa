...
**Son güncelleme:** 2026-08-09 (7. oturum — **M6-06 A FAZI DONE · M6-06 wip · M6 5/12**)
...
>
> **2165 PASS / 0 FAIL / 0 SKIP · 16 paket · 98 test fonksiyonu (4 dosya) · kuşak ağı 18/60 (%30,0)** · `make audit` exit 0. **Sıradaki:** "ŞU AN" → **M6-06 B fazı**.
...
| **Sıradaki görev** | **M6-06 B FAZI** — Wall Tags (plaketler). **Kırmızı çizgi: §4.7** (AES anahtarı ekranda **HİÇ** gösterilmez) **ve §4.4** (replace tag → `last_ctr`/replay). **NFC/plaket/ctr → skill `tappa-sun` ZORUNLU** · **UI → skill `tappa-brand` ZORUNLU** · **migration var → agent `tappa-db-migrator`**. 🔴 **B FAZININ BİLMESİ GEREKEN ALTI ŞEY:** **(1) MIGRATION 00013 ZORUNLU VE İÇERİĞİ KARARA BAĞLANDI** (kullanıcı, 2026-08-08): backlog **T8** (`CHECK (uid ~ '^[0-9A-F]{14}$')` — iki yazım tek AAD; mevcut satırlar zaten büyük harf, **veri taşıması yok**) **+ T9** (`REVOKE UPDATE ON tags` + `GRANT UPDATE (last_ctr, status, retired_at, replaced_by)` **artı monotonluk trigger'ı**; ⚠️ ölçüldü: `AdvanceTagCounter` **zaten katı artış** istiyor (`prev.old_ctr < @ctr`), yani trigger yeni bir kırılma getirmiyor). **T11** (`transactions(tenant_id, location_id)` indeksi) da aynı migration'a aday. **(2) ANAHTAR PANELDEN GEÇMEZ — ENVANTER MODELİ** (kullanıcı, 2026-08-08): plaketi **Tappa encode edip DB'ye yükler**, panel yalnız **BAĞLAR** (eskiyi retire, yeniyi lokasyona ata). Q06 zaten *anahtarı M8-05 runbook'u üretir* diyor ve **Q10 AÇIK** (tedarikçi + anahtar teslimi). Bedeli: **bağlanmamış bir plakete yer gerek** — `tags.tenant_id` ve `location_id` bugün **NOT NULL**. **(3) KARTIN `encoded/pending` KRİTERİ BUGÜN TEMSİL EDİLEMİYOR** — `aes_key_ref bytea NOT NULL`, yani **anahtarsız bir tag satırı var olamaz**; (2) ile birlikte çöz. **T7** o zarfın (44 bayt) şema tarafından **zorlanmadığını** sayıyor. **(4) `tags`'a ÜRETİM INSERT YOLU YOK** (ölçüldü: `tags.sql`'de tek sorgu `AdvanceTagCounter`) — B onu **açan** görev, ve T8'in *bugün sömürülemiyor* koruması **tam olarak buna** dayanıyordu. **(5) A FAZININ KALIPLARI HAZIR, KOPYALA — AMA TAMAMINI:** mutasyon rotası `ProtectWriting` (`floodGate → sameOriginGate → requireAdmin → sessionGate`, **dört aşama, SAY**) · audit **`RecordTx` ile aynı transaction'da** · geri alınamaz eylem → **MAC'li onay v2** (`deactivateconfirm.go`, payload `sürüm|issued|action|subject|session`, **ON İKİ** parça listeli, `mint(action, subjectID, sessionID)`) **artı `owner`-only** · geri alınamaz eylemin bildirimi → **C′ kalıbı** (yönlendirme id taşır, bölüm **aynı istekte** aktörün audit satırını okur; **asla `Done`'a bağlama**) · kapalı sözlük **koddan/şemadan türetilir**, elle liste yazma. ⚠️ Bu kalıbın **yarısını kopyalamak bu projede DÖRT kez** oldu. **(6) KUŞAK AĞI kapsamını koşuda basıyor — bugün 18/60 (%30,0)**; payda her görevde artıyor, **canlı iddia olarak yazma**. |
...
| `make migrate-status` | **00001–00012 uygulanmış**. **00012 = M6-05 B** (`employee_invites.cancelled_at`) — projenin **ilk güvenlik düzeltmesi migration'ı**; sütun düzeyi `GRANT UPDATE (used_at, cancelled_at)`, `Down` **gerçek** (ayrı DB'de 12→11 koşuldu), ve `resolve_invite_by_code_hash` **DROP+CREATE** edilirken `OWNER`/`REVOKE`/`GRANT` **yeniden kuruldu** (DROP üçünü de sessizce atar → ADR 0002 §7 bypass'ı) |
